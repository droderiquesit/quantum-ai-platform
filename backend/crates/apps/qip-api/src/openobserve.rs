...
//!
//! It does not, however, keep dialling a dead peer at full rate. A pass in
//! which neither signal reached the collector doubles the wait before the
//! next, bounded by [`BACKOFF_CEILING`] and never shortened below the
//! configured interval; a pass in which *either* signal was accepted resets it
//! to that interval, because a reachable collector with one bad endpoint is
//! not an outage and slowing the healthy signal to punish the sick one helps
//! nobody. The loop still never stops, and a dropped [`DrainHandle`] is
//! noticed within a quarter of a second rather than after the whole backoff.
//!
