...
pub const ROUTES: &[Route] = &[
    ...
    // The capital grant (ADR 0075; blueprint §36.3's centre column). The
    // one writer of the central plane's envelopes had no production caller,
    // and the plane's own doc forbids a cycle stage being it. This is the
    // operator route that raises the kernel's intent instead: two people,
    // each dated by the presence gate, so in this build it is authorised in
    // shape and refused in fact (ADR 0065, ADR 0076). A grant funds a
    // simulated desk — the envelope bounds paper orders — and the plane can
    // still refuse the pair on the allocation, the rung or a dark region.
    Route {
        method: Method::Post,
        pattern: "/strategies/:strategy/capital-grants",
        required_role: Role::Operator,
        summary: "sign the authenticated operator's name to a capital grant for a strategy on \
                  a capital-holding rung, sized by the allocator; the first signature is held \
                  and the second asks the central plane to issue the envelope, which it may \
                  still refuse — the pair's decision is journalled before the plane is asked",
        success: 200,
    },
    // The recalibration surface (ADR 0061). A proposal is generated by the
    // LEARN stage from regret evidence and can be supplied by nobody; the
    // signature route takes a rationale and nothing else; and what two
    // signatures produce is an artefact for a deployment to commit and
    // mount, never a bound moving under the running process.
    Route {
        method: Method::Get,
        pattern: "/risk/recalibrations",
        required_role: Role::Viewer,
        summary: "the recalibration proposals the platform has generated from counterfactual \
                  regret and are standing open, with every proposal record the log holds — \
                  proposed, withdrawn, enacted — and the name of the limit set this process \
                  runs under",
        success: 200,
    },
    Route {
        method: Method::Post,
        pattern: "/risk/recalibrations/:rule/approvals",
        required_role: Role::Operator,
        summary: "sign the authenticated operator's name to a recalibration proposal the \
                  platform generated; the first signature is held and the second emits the \
                  limit set with the one bound replaced, for a deployment to commit and mount \
...
impl Api {
    ...
    fn dispatch(&self, request: &Request, principal: &Principal, route: &Route) -> Response {
        ...
        match (route.method, route.pattern) {
            ...
            (Method::Post, "/strategies/:strategy/capital-grants") => {
                let Some(strategy) = path_segment(&request.path, 1) else {
                    return Response::json(404, r#"{"error":"no such route"}"#);
                };
                // The same body screen the promotion signature uses: a
                // rationale and nothing else, so the approver cannot be sent
                // and the screen's refusal names the position rather than
                // echoing the key.
                let body = match request
                    .body_as_str()
                    .map_err(|error| error.message().to_string())
                ...
                // The instant the identity is dated at, asked for rather than
                // taken off the principal, for the reason the promotion
                // route gives: a standing bearer token attests nobody's
                // presence, so this refuses and the grant does not happen.
                // The same instant dates the compliance credential the
                // kernel builds, because `ApprovalChain::grant` needs both
                // signers fresh at the instant of issue.
                let authenticated_at =
                    match principal.authentication_instant("signing a capital grant") {
                        Ok(instant) => instant,
                        Err(refusal) => {
                            return Response::json(
                                403,
            ...
            (Method::Get, "/risk/recalibrations") => {
                let (status, body) = crate::ledger_views::render_fallible(
                    crate::rule_views::recalibrations(&platform),
                );
                Response::json(status, body)
            }
            (Method::Post, "/risk/recalibrations/:rule/approvals") => {
                let Some(rule) = path_segment(&request.path, 2) else {
                    return Response::json(404, r#"{"error":"no such route"}"#);
                };
                let body = match request
                    .body_as_str()
                    .map_err(|error| error.message().to_string())
                    .and_then(crate::rule_views::RecalibrationApprovalRequest::parse)
                {
                    Ok(body) => body,
                    Err(reason) => {
                        return Response::json(400, crate::registration_views::refusal(&reason));
                ...
                // The approver is the authenticated principal and never the
                // body — the rule the promotion signature above keeps, and
                // the one that makes this an approval rather than a claim to
                // have been approved.
                //
                // Neither `now` nor `principal.issued_at`, for the reason
...
mod tests {
    ...
    -> Result<()> {
        ...
        // may not trade on — and since ADR 0061 a second: every limit in the
        // boot set reads as not dormant, so a rule that never fires is a
        // zero and not an absent series. The honest state before the first
        // cycle is exactly those two gauges and no cycle series, not an
        // empty page.
        ...
        assert!(
            before.contains("\nqip_rule_dormant{rule=\"order-notional\"} 0\n"),
            "assembly records every limit as not dormant, and the scrape must carry it: {before}"
        );
        assert!(
        ...
        assert_eq!(
            ...
            2,
            "before the first cycle the only series are the two assembly gauges: {before}"
        );
