...
mod tests {
    ...
    fn a_limits_file_that_only_widens_a_bound_boots_and_the_banner_names_which_one_moved() {
        ...
        // hash — a reviewer reading the health output could not tell the
        // running set apart from the shipped one without hashing the file
        // themselves. `limits_banner` now names the moved limit.
        let shipped = LimitSet::conservative_default();
        let moved = shipped
            .rebound("order-notional", 300_000.0)
            .expect("a known limit rebounds");
