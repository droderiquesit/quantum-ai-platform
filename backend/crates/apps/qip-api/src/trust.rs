...
//! capital at risk. The kernel names the gap; this module is where the API
//! closes it, exactly as the brain binaries do in their own `trust` modules.
//!
//! For this process the stakes are one step higher than for a brain: the API
//! is the binary that *dispatches* envelopes down the mesh (see
//! [`crate::mesh`]), so the key installed here is the key every cell's
//! downlink verifies those grants against. A cell rejects a grant signed
//! under any other key, which is why the variable is one name shared by both
//! ends rather than two settings that could drift.
//!
...
mod tests {
    ...
    fn a_short_key_is_refused_by_the_plane_and_the_refusal_says_why() -> Result<()> {
        ...
        // this test pins that the refusal actually propagates rather than
        // being swallowed by the swap.
        let mut platform = platform()?;
