...
//!
//! Nothing, today. ADR 0024 does put the egress proxy beside this process, so
//! unlike the fast brain it has a loopback hop that could terminate TLS — but
//! `infrastructure/egress/envoy.yaml` declares listeners for the Google APIs,
//! Vertex and the two IBM Quantum hosts and **none for OpenObserve**, so there
//! is no address to point this at. `qip_transport::Url::parse` refuses every
//! scheme but plaintext `http` by name, so ADR 0030's collector — anonymous on
//! the public internet over https — is refused at start-up with an `EX_CONFIG`
//! exit rather than dialled for ever.
//!
