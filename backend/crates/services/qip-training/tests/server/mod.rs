...
//!
//! The Vertex adapter is only interesting where it meets a socket, so the tests
//! give it one rather than a double. The properties worth asserting are all
//! about what happens when an answer is *not* a job record — a peer that goes
//! quiet, a 401, a state string this decoder will not map — and none of them
//! can be reached by stubbing out the transport.
//!
//! Each route holds a list of actions consumed in turn, with the last
//! repeating, because the central property here is a *sequence*: a submit that
//! could not be read, then a reconciliation that finds nothing, then one that
//! finds the job. Anything unrouted answers 404, so a test whose adapter asks
//! for a path nobody scripted fails on that path instead of hanging.
//!
//! Requests are recorded in full, headers and body, because the claim that
//! matters most is about what left the process: that the bearer token
//! travelled in a header and never in a URL.
//!
...
impl TestService {
    /// A service answering the routes given, and 404 for anything else.
            ...
            while !thread_stop.load(Ordering::SeqCst) {
                match listener.accept() {
                    Ok((stream, _)) => {
                        ...
                        // listener ready, exactly as it would against the real
                        // service.
    ...
    /// Every request the service took for one method and path.
