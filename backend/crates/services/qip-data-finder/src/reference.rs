...
/// The scheme a replay-under-admission's locator carries, so a ledger entry
/// says on its face that the bytes were read from a file rather than fetched
/// from the vendor. See [`DataReference::of_admitted_replayed`].
pub const REPLAY_LOCATOR_SCHEME: &str = "replay://";
...
pub enum SourceOrigin {
    ...
    Discovered,
    /// Admitted by the licensing catalogue for a shipped connector, and
    /// fetched from that connector by this platform.
    CatalogueAdmitted,
    /// A replay of bytes somebody says were recorded from a
    /// catalogue-admitted connector, researched under that connector's
    /// licence. The licence is the gate's answer; the bytes are the file's
    /// author's word, and the origin says so: a reader who could not tell a
    /// replay-under-admission from a live fetch would credit a hand-written
    /// file with a vendor's standing. See [`Self::is_independent_vendor`].
    ReplayedAdmitted,
...
impl SourceOrigin {
    pub const fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::ReplayedAdmitted => "replayed_admitted",
    ...
    ///
    /// A replay under a connector's admission is not one either, and this
    /// is the more consequential exclusion. Nothing about a replay file
    /// proves its bytes came from the vendor its header names; a
    /// hand-written bars file headed with one admitted connector, restarted
    /// and headed with another, put two "vendors" behind a subject on the
    /// ledger and opened rule 31's hold on zero vendor bytes — with the
    /// first vendor's licence attributed to fabricated bars on the
    /// permanent log. Only a fetch this platform made from the vendor
    /// counts, because only that fetch is evidence the vendor served it.
...
/// already has.
///
/// On the wire the same invariants hold: deserialisation goes through
/// [`Self::build_hashed`], so a frame on the event log edited to name no
/// symbols, no locator, zero bytes or an availability outside `[0, 1]` is
/// refused at the read rather than restored into the ledger. The derive it
/// replaces accepted all four until 2026-09-12 — the one constructor no
/// gate stood in front of.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
#[serde(try_from = "DataReferenceWire")]
...
impl TryFrom<DataReferenceWire> for DataReference {
    ...
    fn try_from(wire: DataReferenceWire) -> Result<Self> {
        if wire.content_hash.trim().is_empty() {
            return Err(Error::invalid(format!(
                "the data reference for `{}` on the wire carries no content hash, and a \
                 reference with no hash can flag no revision; it is refused rather than restored",
                wire.source_id
            )));
        }
        if wire.category.is_none() && wire.origin != SourceOrigin::Generated {
            return Err(Error::invalid(format!(
                "the data reference for `{}` on the wire came through the `{}` door and names no \
                 §7.6.1 category; only a generated stream may, so this is refused rather than \
                 restored",
                wire.source_id,
                wire.origin.as_str()
            )));
...
impl DataReference {
    ...
    /// Describe a replay of bytes somebody says were recorded from the
    /// catalogue-admitted `source`, researched under its admission.
    ///
    /// The same category and schema as [`Self::of_admitted`] — the licence
    /// is the gate's — and a different door: [`SourceOrigin::ReplayedAdmitted`],
    /// which [`SourceOrigin::is_independent_vendor`] does not count. A
    /// composition root that gave a replay a connector's name used to
    /// reference its bytes through `of_admitted`, and the ledger then held a
    /// vendor's standing for whatever the file contained. The locator is
    /// prefixed `replay://` so an auditor reading the ledger can tell a
    /// replay-under-admission from a fetch this platform made.
    #[allow(clippy::too_many_arguments)]
    pub fn of_admitted_replayed(
        source: &AdmittedSource,
        locator: impl Into<String>,
        symbols: impl IntoIterator<Item = String>,
        range: DataPeriod,
        ...
        let locator = if locator.starts_with(REPLAY_LOCATOR_SCHEME) {
            locator
        } else {
...
mod tests {
    ...
    /// The wire is gated the way the constructors are: a reference edited to
    /// name no symbols, no locator, zero bytes, an availability past one or
    /// no hash is refused at the read, and an honest one round-trips. Until
    /// 2026-09-12 the derive was a constructor with no gate in front of it,
    /// and a frame on the log could restore into the ledger what
    /// `build_hashed` had refused to build.
    ///
    /// Mutated by replacing the `try_from` body with a plain field copy —
    /// confirmed every forged half then deserialises and this fails, then
    /// restored.
    #[test]
    fn a_reference_off_the_wire_is_held_to_the_constructors_refusals() -> Result<()> {
        let admitted = admitted_frankfurter()?;
        let reference = DataReference::of_admitted(
            &admitted,
            admitted.endpoint(),
        ...
        let honest = serde_json::to_value(&reference)?;
        // Premise: the honest form round-trips.
        ...
        for (field, forged) in [
            ...
            wire[field] = forged;
            assert!(
                serde_json::from_value::<DataReference>(wire).is_err(),
                "a reference with a forged `{field}` was restored off the wire"
            );
        }
        Ok(())
    }

    /// The replayed door: the same licence and category as the catalogue
    /// door, a locator that says `replay://` on its face, and an origin the
    /// concentration rule does not count. The scenario this pins is the one
    /// the review found: a hand-written bars file headed with one admitted
    /// connector, then with another, must never read as two vendors.
    ///
    /// Mutated by making `is_independent_vendor` include `ReplayedAdmitted`
    /// — confirmed the vendor-count assertion then fails, then restored.
    #[test]
    fn a_replay_under_a_connectors_admission_is_not_a_vendor() -> Result<()> {
        let admitted = admitted_frankfurter()?;
        let replayed = DataReference::of_admitted_replayed(
            &admitted,
            "bars://frankfurter-ecb-reference-rates/BTC-USD?interval=1m",
            ["BTC-USD".to_string()],
            DataPeriod::instant(now()),
            b"[{\"close\":\"100\"}]",
            now(),
        ...
        assert_eq!(replayed.origin(), SourceOrigin::ReplayedAdmitted);
        assert_eq!(
            replayed.category(),
            Some(SourceCategory::GovernmentAndTrade),
            "the licence and category are the gate's, not the file's"
        );
        assert!(
            replayed.locator().starts_with(REPLAY_LOCATOR_SCHEME),
            "an auditor must be able to tell a replay from a fetch on the ledger: {}",
            replayed.locator()
        );
        assert!(
            !SourceOrigin::ReplayedAdmitted.is_independent_vendor(),
            "replayed bytes were counted as a vendor"
        );
