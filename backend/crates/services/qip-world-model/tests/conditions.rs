...
fn a_condition_failure_against_a_link_the_graph_never_claimed_marks_nothing() -> Result<()> {
    ...
    let marked = graph.record_condition_failure("CCC", "DDD", "bear/stressed", at(2_000))?.marked;
    assert_eq!(
        marked, 0,
        "a link nobody claimed was reported as having been marked"
    );
    let marked = graph.record_condition_failure("AAA", "BBB", "bear/stressed", at(2_000))?.marked;
    assert_eq!(
        marked, 1,
...
fn a_condition_failure_is_not_written_onto_an_edge_that_was_not_yet_knowable() -> Result<()> {
    ...
    let marked = graph.record_condition_failure("AAA", "BBB", "bear/stressed", at(1_000))?.marked;
    ...
    let marked = graph.record_condition_failure("AAA", "BBB", "bear/stressed", at(9_000))?.marked;
