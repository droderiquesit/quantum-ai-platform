...
fn a_rung_is_judged_by_its_widest_and_its_tightest_quote_and_not_by_one_record() {
    // The tight record on the upper rung is listed first; only the two at
    // 300bps invert, and only against `obj-tight-below`, which is listed last.
    // `obj-wide` and `obj-zwide` are quoted identically on purpose: with a tie
    // for widest, which of the two the refusal names is decided by the
    // tie-break rather than by arrival order, and the reversed pass below is
    // what proves that.
    let quotes = [
        ("obj-narrow", Rung::ListedEquityAndFutures, 5.0),
        ("obj-wide", Rung::ListedEquityAndFutures, 300.0),
        ("obj-zwide", Rung::ListedEquityAndFutures, 300.0),
    ...
    // The same five records in the reverse order name the same pair. A refusal
    // a replay could attribute to a different record is not a replay, and a
    // catalogue is a file whose order nobody guarantees.
