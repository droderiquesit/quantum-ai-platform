...
//!
//! The hosted provider is only interesting where it meets a socket, so the
//! tests give it one rather than a double. The claim that matters most here —
//! that a backend the service calls a simulator produces no result at all —
//! can only be made against a service that says so, and the claim beside it —
//! that a job is polled until the service reports it finished — is a
//! *sequence*: the same route answers "Queued" and then "Completed". So each
//! route holds a list of actions consumed in turn, with the last repeating.
//!
//! Anything unrouted answers 404, so a test whose adapter asks for a path
//! nobody scripted fails on that path rather than hanging. Requests are
//! recorded in full, headers and body, because the other claim worth making is
//! about what left the process: that the API token travelled in a header and
//! never in a URL.
//!
