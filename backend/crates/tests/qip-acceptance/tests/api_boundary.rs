...
fn every_mutating_route_is_reviewed_here_and_each_raises_a_typed_intent() {
    ...
    let mutating: BTreeSet<(String, String)> = routes
        ...
        .collect();
    // The fourth is the venue-registration approval: an operator intent
    // raised with the authenticated identity, like clearing the kill
    // switch, whose typed intent is `Platform::approve_registration` —
    ...
    // The approval carries the same verified identity into the kernel's one
    // registration path, and the body is screened by the manifest's shape
    // rule before that path is reached — a key-shaped `secret` never gets
    // as far as the kernel, let alone the log.
    assert!(
...
fn the_api_calls_no_platform_mutator_it_has_not_been_allowed() {
    ...
    //   session exactly as clearing the kill switch does, and the kernel
    //   takes the record's operator from that identity and from nothing the
    //   body says. Nothing a caller sends becomes a credential — the body
    //   carries the terms the operator read and a deployment *variable
    //   name*, screened by the manifest's `SecretRef` shape rule before the
    //   kernel sees it, and the value stays in Secret Manager where the
    //   operator put it. The record is journalled before it stands.
    // * `decide_eligibility` — the same class as `approve_registration`, and
    //   admitted on the same terms. `POST /ledger/users/:user/eligibility`
    //   records an operator's decision about whether one investor may be
    //   funded; the route builds an `OperatorIdentity` from the authenticated
    //   session and the kernel takes the deciding operator from that identity
    //   and from nothing the body carries, so a caller cannot name who
    //   decided. What the body does carry is the decision and its reason,
    //   which are the operator's own words and are journalled with the
    //   subject before the registry adopts them. The decision is a refusal by
    //   default: `fund_user` is refused until a standing grant exists, so
    //   this route can only ever be the act that lifts a refusal a person
    //   took responsibility for, never one that creates capital.
