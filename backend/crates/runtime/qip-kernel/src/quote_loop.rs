...
impl QuoteLoopReview {
    ...
    pub fn describe(&self) -> Option<String> {
        if self.considered == 0 {
            // Say so rather than falling silent.
            //
            // This returned `None` until 2026-09-14. A cycle considers no
            // instrument whenever nothing it holds has a two-sided book, which
            // is every cycle of a freshly opened platform and every cycle of
            // the acceptance fixture — so the stage carried no trace of the
            // quote loop in the only state that exists today, and
            // disconnecting the call site entirely left this crate's whole
            // suite green. A reader could not tell a loop that ran and found
            // nothing quotable from a loop nobody had wired in.
            //
            // This is the third module in one day to have shipped that shape:
            // the family review answered it by writing its gauge every cycle
            // including when every arm is zero, and the adaptive cadence by
            // naming what it held. The failure is not in any one of them — it
            // is that "produce nothing when there is nothing" reads as
            // obviously right and quietly removes the evidence that the code
            // runs at all.
            return Some(
                "quote loop considered no instrument: none held has a two-sided book".to_string(),
            );
        }
...
mod tests {
    ...
    fn the_stage_outcome_carries_the_pass_so_the_review_cannot_be_computed_and_dropped() {
        ...
        // And a platform that has observed nothing says that it considered
        // nothing, rather than adding nothing at all.
        //
        // This asserted `untouched.detail == before.detail` until 2026-09-14,
        // on the ground that "0 quoted, 0 withheld" every quiet cycle is noise
        // hiding the cycle where the sentence means something. That argument
        // is right about *that* sentence and wrong about silence: a platform
        // that has observed nothing is the state every cycle of a fresh
        // deployment is in, so the loop reached no surface at all in the only
        // state that exists, and disconnecting its call site from `stage_act`
        // left this crate's entire suite green. The line below is not a count
        // of zeros — it names the reason nothing was quotable, which is a fact
        // an operator can act on, and it cannot appear on a cycle that
        // considered anything.
        let quiet = fixture();
        let untouched = review(&quiet, start(), before.clone());
        assert_ne!(
            untouched.detail, before.detail,
            "a quiet cycle left no trace, so nothing distinguishes a loop that ran from a loop \
             nobody wired in"
        );
        assert!(
            untouched.detail.contains("considered no instrument"),
            "a quiet cycle did not say why it quoted nothing: {}",
            untouched.detail
        );
    }
}
