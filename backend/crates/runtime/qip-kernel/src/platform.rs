...
pub struct Platform {
    ...
    /// Recalibration proposals standing open, by rule: generated by
    /// [`Self::review_rules`] from regret evidence, withdrawn by it when the
    /// evidence stops clearing the bar, and closed by
    /// [`Self::approve_recalibration`] on the second signature. Resumed from
    ...
    pending_recalibrations: BTreeMap<String, qip_contracts::governance::Approval>,
...
/// The producer on every recalibration signature, told apart from the
...
///
/// **Every one of the four judges a record alone, and one property does not
/// fit that shape.** Whether two records can sit on one ladder is a relation
/// between them: 300bps on a small-cap and 250bps on a negotiated holding
/// beneath it are each admissible here and impossible together.
/// `qip_financial::ladder::prove_quotes_can_coexist` asks that question of the
/// whole map, immediately after this runs in `Platform::new`. It is separate
/// rather than folded in because a refusal naming one record would be telling
/// the operator to correct a record that may be the right one.
...
///
/// **Enactment is an artefact, not a mutation.** `artefact` is the running
/// limit set with exactly one bound replaced, rendered for a deployment to
/// commit under `data/risk-limits/` and mount as `QIP_RISK_LIMITS_PATH`
/// (ADR 0061). The process that produced it keeps the limits it booted
/// with: there is no method on [`Platform`], the checker or the monitor that
/// installs a set after boot, and the acceptance suite scans for one. Two
/// signatures therefore authorise a *file*, which is reviewed like any
/// other, rather than a live control moving under a running book.
///
/// Why this can only loosen, and why nothing tightens automatically: the
/// bound comes from a platform-generated [`RecalibrationProposal`], whose
/// constructor refuses a non-loosening bound; the route body carries a
/// rationale and nothing else; the defence finding (§12.3's second row) has
/// no bound field and no route; and the only path by which a bound changes
/// in a running process is the file read at boot, so a tightening is a
/// reviewed commit to that file — the path a desk already has.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize)]
pub struct RecalibrationApprovalEntry {
    pub rule: String,
    ...
    pub second_approver: Option<String>,
    pub rationale: String,
    /// `awaiting_countersignature`, `enacted`, or `refused`.
    pub outcome: String,
    /// Present on a refusal: what the limit set said about the rebound.
    pub detail: Option<String>,
    /// Present on `enacted`: the set to commit and mount.
    pub artefact: Option<LimitSet>,
    pub at: Timestamp,
}

impl EventBody for RecalibrationApprovalEntry {
    /// A governance decision about a control, in the Decide group the log
    /// never evicts, told apart from its neighbours by producer. See
    /// [`RECALIBRATION_APPROVAL_ORIGIN`].
    const TOPIC: Topic = Topic::ComplianceEvaluated;
    const SCHEMA_VERSION: u32 = 1;
}
...
impl Platform {
    fn stage_simulate(&mut self, now: Timestamp) -> StageOutcome {
        ...
        match &report {
            Some(report) => {
                detail.push_str(&format!("; stress: {}", report.summarise()));
                // One gauge per scenario, under the scenario's own name and
                // from the same report the platform holds, so the chart and
                // `Platform::stress_report` cannot tell two different
                // stories about one pass. Before the factor model existed
                // this series carried a single scenario at a full (beta 1.0)
                // exposure; it now carries the library at the measured beta,
                // and a position the factor cannot measure contributes
                // nothing to it rather than being counted immune.
                for result in &report.scenarios {
                    self.telemetry.metrics.gauge(
                        names::SIMULATION_STRESS_LOSS_FRACTION,
                        labels([("scenario", result.scenario.as_str())]),
            ...
            // is a different fact from a stress that found nothing.
            None => detail.push_str("; no open position to apply a scenario to"),
        }
    ...
    fn attribute(&mut self, now: Timestamp) -> (StageOutcome, BTreeMap<String, Decimal>) {
        ...
        // 0058). Computed once for the whole attribution rather than per
    ...
    fn review_rules(&mut self, now: Timestamp) -> (Option<String>, Vec<String>) {
        ...
        let mut problems = Vec::new();
        let regrets = regret_by_rule(&self.declined_scores);

        // §12.3's first row: propose, never apply. A rule whose sample
        // clears the bar on the regret side and has no proposal standing
        // gets one, built from the bound that would have admitted every
        // regretted path; a rule whose standing proposal no longer clears
        // the bar has it withdrawn, so a signature cannot land on evidence
        // that has since evaporated.
        for (rule, regret) in &regrets {
            ...
            let Some(proposed_bound) = regret.admitting_bound else {
                problems.push(format!(
                    "{rule} vetoed mostly profitable paths but no reading of it was carried, so \
                     no admitting bound can be proposed"
                ));
                continue;
            };
            let proposal = match RecalibrationProposal::new(
                ...
                &kind,
                proposed_bound,
                RegretEvidence::of(regret),
                now,
            ) {
                Ok(proposal) => proposal,
                Err(error) => {
                    problems.push(format!(
                        "a recalibration of {rule} was indicated and refused: {}",
                        error.message()
                    ));
                    continue;
                }
        ...
        let standing: Vec<String> = self.open_proposals.keys().cloned().collect();
        for rule in standing {
            if regrets.get(&rule).is_some_and(RuleRegret::is_too_tight) {
                continue;
            }
            let Some(proposal) = self.open_proposals.get(&rule).cloned() else {
                continue;
            };
            match self.journal_once(
                proposal.with_outcome(PROPOSAL_WITHDRAWN, now),
                RULE_REVIEW_ORIGIN,
                now,
            ) {
                Ok(_) => {
                ...
                Err(error) => problems.push(format!(
                    "the withdrawal of {rule}'s recalibration could not be journaled: {}",
                    error.message()
                )),
            }
        }
    ...
    /// The recalibration proposals standing open, by rule — read-only; one
    /// reaches here through [`Self::review_rules`] and nothing else.
    pub fn open_recalibrations(&self) -> &BTreeMap<String, RecalibrationProposal> {
        &self.open_proposals
    }

    /// Every recalibration record the log holds from this kernel's review,
    /// oldest first — proposed, withdrawn and enacted alike — so a reader
    /// can see what was proposed and what became of it.
    pub fn recalibration_history(&self) -> Result<Vec<RecalibrationProposal>> {
        Self::recalibration_records(&self.event_log)
    }

    fn recalibration_records(log: &EventLog) -> Result<Vec<RecalibrationProposal>> {
        log.by_topic(RecalibrationProposal::TOPIC)
            .into_iter()
            .filter(|event| event.lineage.producer == RULE_REVIEW_ORIGIN)
            .map(|event| {
                StreamEnvelope::from_frame(event)
                    .and_then(|envelope| envelope.decode::<RecalibrationProposal>())
                    .map(|envelope| envelope.body)
            })
            .collect()
    }
    ...
    /// have.
    ///
    /// Cloned from [`Self::approve_promotion`] line for line where the two
    /// are about the same thing — a fresh credential, two distinct people,
    /// a first signature that goes stale — and different where they are
    /// not. The subject is a *proposal the platform generated*: a rule with
    /// none standing is refused as not found, and the refusal says the
    /// proposal cannot be supplied, because a bound a caller could name
    /// would be a request to loosen a control and not an approval of the
    /// evidence for one.
    ///
    /// **The second signature enacts nothing in this process.** It produces
    /// [`RecalibrationApprovalEntry::artefact`] — the running set with the
    /// one bound replaced, through `LimitSet::rebound` — journals the
    /// proposal as enacted, and closes it. `self.orders`, `self.monitor`
    ...
    pub fn approve_recalibration(
        ...
        rationale: &str,
        now: Timestamp,
    ) -> Result<RecalibrationApprovalEntry> {
        if !operator.is_fresh(now, PROMOTION_CREDENTIAL_AGE) {
            return Err(Error::denied(format!(
                "operator {} authenticated more than {:?} ago; re-authenticate to sign a \
                 recalibration that loosens a risk limit",
                operator.subject(),
                PROMOTION_CREDENTIAL_AGE
            )));
        }
        let proposal = self.open_proposals.get(rule).cloned().ok_or_else(|| {
            Error::not_found(format!(
                "the platform has proposed no recalibration of {rule}; a proposal is generated \
                 from regret evidence and cannot be supplied"
            ))
        })?;

        match self.pending_recalibrations.get(rule).cloned() {
            None => {
                let approval = qip_contracts::governance::Approval::new(
                    rule,
                    operator.subject(),
                ...
                let entry = RecalibrationApprovalEntry {
                    rule: rule.to_string(),
                    current_bound: proposal.current_bound,
                    proposed_bound: proposal.proposed_bound,
                    approver: approval.approver.clone(),
                    second_approver: None,
                    rationale: approval.rationale.clone(),
                    outcome: "awaiting_countersignature".to_string(),
                ...
                self.journal_record(entry.clone(), RECALIBRATION_APPROVAL_ORIGIN, now)?;
                self.pending_recalibrations
                    .insert(rule.to_string(), approval);
                Ok(entry)
            }
            Some(first) => {
                if first.at.saturating_add(PROMOTION_APPROVAL_WINDOW) < now {
                    self.pending_recalibrations.remove(rule);
                    return Err(Error::denied(format!(
                        "the first signature on {rule}'s recalibration was given at {} and a \
                         countersignature must follow within {:?}; it has been discarded and \
                         both signatures must be given again",
                        first.at.to_rfc3339(),
                        PROMOTION_APPROVAL_WINDOW
                ...
                if first.approver == operator.subject() {
                    return Err(Error::denied(format!(
                        "{} has already signed {rule}'s recalibration; a dual approval needs \
                         two people, and a second session is not a second person",
                        operator.subject()
                    )));
                }
                ...
                // their answer.
                self.pending_recalibrations.remove(rule);
                let artefact = self.monitor.limits().rebound(rule, proposal.proposed_bound);
                let mut entry = RecalibrationApprovalEntry {
                    rule: rule.to_string(),
                    current_bound: proposal.current_bound,
                    proposed_bound: proposal.proposed_bound,
                    approver: approval.approver.clone(),
                    second_approver: approval.second_approver.clone(),
                    rationale: rationale.to_string(),
                    outcome: PROPOSAL_ENACTED.to_string(),
                    detail: None,
                    artefact: None,
                    at: now,
                };
                match &artefact {
                    Ok(set) => {
                        self.journal_record(
                            proposal.with_outcome(PROPOSAL_ENACTED, now),
                            RULE_REVIEW_ORIGIN,
                            now,
                        )?;
                        self.open_proposals.remove(rule);
...
mod simulate_tests {
    ...
    fn an_open_position_is_stress_tested_against_the_standard_library() {
        ...
        // Computed independently of `stress_the_book`, from the scenario's
        ...
        // would remain, a few basis points against a shock of thousands —
        // and not only "some loss happened", which a liquidity charge alone
        // would also satisfy. With one instrument on the tape the market factor *is*
        // that instrument's return, so its measured beta is exactly one and
        // the expectation below is the full-beta loss. The exit charge reads
        // the same cost model the stage prices it off — the frictions that
        // do not depend on participation — rather than a figure written
        // here, which would be a second claim about the cost of trading.
        let equity = platform.capital.equity().to_f64();
        let notional = 100.0 * 100.0;
        let equity_shock = scenario
            .shocks
            .iter()
            .find(|shock| shock.factor == qip_risk::market_factor::EQUITY_SHOCK)
            .expect("the scenario names an equity shock")
            .magnitude;
        let total_pnl = notional * equity_shock;
        let costs = CostModel::liquid_equity();
        let exit_bps = costs.commission_bps + costs.half_spread_bps + costs.tax_bps;
        let liquidation_cost = notional * exit_bps * scenario.liquidity_multiplier / 10_000.0;
...
#[cfg(test)]
mod liquidity_ladder_tests {
    //! The liquidity floor stops being a rationale attached to nothing.
    //!
    //! `LimitKind::MinLiquidity` — the `liquidity` limit in
    //! `LimitSet::conservative_default`, rationale "most of the book must be
    //! exitable within a week" — reads `RiskState::liquidatable_within`.
    //! Nothing in this kernel wrote that map: `RiskAggregates::mark_liquidity`
    //! had no caller outside its own crate's tests, and `risk_state_from`
    //! never called `RiskState::with_liquidity_horizons`. The limit took its
    //! `None` arm on every book of every cycle. That is the failure
    //! `.claude/rules/domains/risk-and-execution.md` names by example, and it
    ...
    const FAST: &str = "obj-FAST";
    const SLOW: &str = "obj-SLOW";

    fn start() -> Timestamp {
        Timestamp::from_secs(1_760_000_000)
    }

    /// Two names, one on each side of a week.
    ///
    /// `FAST` is an ordinary listed equity: `Rung::classify` puts it on
    /// `ListedEquityAndFutures`, whose horizon is same-day. `SLOW` is the same
    /// instrument type carrying a negotiated liquidity profile, which
    /// `Rung::classify` pushes down to `PrivateCreditAndRealAssets` — months.
    ...
    #[test]
    fn the_liquidity_floor_limit_can_actually_fire() {
        // A book nine tenths of which takes months to leave. The conservative
        // default demands 0.80 exitable within five days; this book offers
        // 0.10, so the floor must breach. Before the ladder was wired the
        // identical book produced no breach at all, because the map the limit
        // reads was empty.
        let platform = platform_with(dec!("100000"), dec!("900000"));
        let state = platform.risk_state();

        // The premise, before the conclusion. An empty map is exactly the
        // state this test exists to end, and a `check` against it would report
        // no breach for the wrong reason — which is what it did.
        let fraction = state
            .liquidatable_within
            .get("5")
            .copied()
            .unwrap_or_else(|| {
                panic!(
                    "the fraction exitable within five days was not computed for the horizon the \
                     default limit names; the map holds {:?}",
        ...
        assert!(
            ...
            "a book of 100k same-day against 900k months should read 0.10 exitable within a \
             week, not {fraction}"
        );
        ...
        let breach = check
            ...
            .unwrap_or_else(|| {
                panic!(
                    "the liquidity floor recorded no breach against a book 90% of which takes \
                     months to leave; the check recorded {:?}",
                    check
                        .breaches
                        .iter()
                        .map(|breach| breach.limit_name.as_str())
        ...
        assert!(
            breach.blocks(),
            "a liquidity floor missed by seven tenths should block, not warn: {breach:?}"
        );
    }

    #[test]
    fn a_book_that_is_exitable_within_the_week_does_not_breach_the_liquidity_floor() {
        // The other half of a working gate. A floor that refuses every book is
        // not a control, it is an outage, and a test that only ever proves the
        // refusal cannot tell the two apart.
        // Comfortably clear, not marginally: the conservative default warns
        // at 0.85 of its floor — 0.941 exitable — so a book at 0.90 would
        // still raise a warning, and a test written against that number would
        // be asserting the warning band rather than the pass.
        let platform = platform_with(dec!("990000"), dec!("10000"));
        let state = platform.risk_state();
        ...
        let fraction = state
            ...
            .get("5")
            .copied()
            .expect("the fraction exitable within five days is computed");
        assert!(
        ...
        assert!(
            ...
            "a book 90% exitable within a week breached a floor set at 80%: {:?}",
            check.breaches
        );
    }
    ...
    #[test]
    fn a_holding_the_platform_has_no_liquidity_record_for_refuses_the_ladder_rather_than_omitting_it()
     {
        // Dropping the unknown holding would take it out of the numerator and
        // the denominator together, so a book made entirely of unknowns would
        // report itself perfectly exitable. The refusal names the instrument.
        let mut platform = platform_with(dec!("100000"), dec!("900000"));
        platform
            .aggregates
            .apply_fill("alpha", "obj-UNKNOWN", &BTreeMap::new(), dec!("500000"))
            .expect("the fill applies");
        ...
        let error = platform
            .liquidity_ladder(&platform.aggregates)
            .expect_err("a book holding an instrument with no liquidity record is refused");
        assert!(
            error.message().contains("obj-UNKNOWN"),
            "the refusal must name the holding it could not place: {}",
            error.message()
        );

        // And the refusal leaves the map empty rather than a fabricated zero,
        // which would read as a book measured and found completely illiquid.
        let state = platform.risk_state();
        assert!(
    ...
    #[test]
    fn a_refused_ladder_puts_the_unevaluated_liquidity_floor_on_the_cycle_report() {
        // The refusal above is silent inside `risk_state`, and a silent
        // liquidity floor is the defect this wiring closes, not a smaller
        // version of it. ACT names it, so an operator reading the cycle sees
        // that the floor went unevaluated and why.
        let mut platform = platform_with(dec!("100000"), dec!("900000"));
        let correlation = CorrelationId::from_string("test-correlation");

        let clean = platform.stage_act(start(), &correlation);
        assert!(
            !clean
                .problems
                .iter()
                .any(|problem| problem.contains("liquidity floor")),
            "a book the ladder accepts reported the floor unevaluated: {:?}",
            clean.problems
        );
        ...
        platform
            ...
            .apply_fill("alpha", "obj-UNKNOWN", &BTreeMap::new(), dec!("500000"))
            .expect("the fill applies");
        let outcome = platform.stage_act(start(), &correlation);
        assert!(
            outcome.problems.iter().any(|problem| problem
                .contains("the liquidity floor was not evaluated")
                && problem.contains("obj-UNKNOWN")),
            "ACT did not report the unevaluated liquidity floor: {:?}",
            outcome.problems
        );
    }
    ...
    fn a_reference_record_stating_an_impossible_spread_stops_assembly() {
        ...
        // cost more than it raises, which `LiquidityLadder::new` refuses. Left
        // to be discovered there it would take the whole book's liquidity read
        // down every cycle for a reason no risk report could show. Refused at
        // start-up instead, naming the record.
        let mut universe = Universe::new();
        universe
            .insert(
                FinancialObject::builder(
        ...
        .expect_err("a record stating a 100% spread stops assembly");
        assert!(
            error.message().contains(FAST) && error.message().contains("10000"),
    ...
    fn an_ordinary_small_cap_quoted_at_three_hundred_bps_no_longer_stops_the_desk() {
        ...
        assert!(
            platform_quoting_or_refusal(300.0, 250.0, dec!("990000"), dec!("10000")).is_err(),
    ...
    fn two_reference_records_whose_exit_costs_cannot_coexist_are_refused_at_assembly_naming_both() {
        ...
        assert!(
            platform_quoting_or_refusal(300.0, 900.0, dec!("990000"), dec!("10000")).is_ok(),
            "the premise failed: a universe whose quotes widen as the ladder descends was \
             refused too"
        );

        // `.err()` rather than `expect_err`, which would print a whole
        // `Platform` on failure and bury the thing being asserted.
        let error = platform_quoting_or_refusal(300.0, 250.0, dec!("990000"), dec!("10000"))
            .err()
            .unwrap_or_else(|| {
                panic!("a listed name quoted wider than the rung beneath it assembled")
            });
...
mod rule_review_tests {
    ...
    fn operator(name: &str, at: Timestamp) -> OperatorIdentity {
        OperatorIdentity::verified(name, "hardware-token", at)
    }

    const WHY: &str = "the regret evidence was reviewed against the desk's mandate";

    /// Twelve regretted paths charged to [`RULE`], each observed at 300,000
    /// against the shipped 250,000: the bar cleared on the regret side.
    fn regret_twelve(platform: &mut Platform) {
        platform.declined_scores.clear();
        for index in 0..12 {
            platform
                .declined_scores
                .push(score(index, true, dec!("500")));
        }
    }

    #[test]
    fn thin_evidence_yields_neither_a_proposal_nor_a_defence() {
        // Nine scored refusals, one short of the sample bar, pointing all
        // one way and then all the other. Recalibrating on nine is
        // recalibrating on noise, and a defence on nine is the same claim in
        // the other direction.
        let mut platform = platform();
        for index in 0..(COUNTERFACTUAL_SIZING_MIN_SAMPLE - 1) {
            platform
                .declined_scores
                .push(score(index, true, dec!("500")));
        }
        let (summary, problems) = platform.review_rules(start());
        assert!(problems.is_empty(), "{problems:?}");
        assert_eq!(summary, None, "thin regret evidence produced a finding");
        ...
        let (summary, problems) = platform.review_rules(start());
        assert!(problems.is_empty(), "{problems:?}");
        assert_eq!(summary, None, "thin defence evidence produced a finding");
        assert!(defences(&platform).is_empty());
    }

    #[test]
    fn a_rule_that_vetoed_mostly_profitable_paths_yields_a_proposal_that_is_never_applied() {
        let mut platform = platform();
        let boot = platform.limits().clone();
        ...
        assert_eq!(written.len(), 1, "{written:?}");
        let proposal = &written[0];
        assert_eq!(proposal.rule, RULE);
        assert_eq!(proposal.outcome, "proposed");
        assert!(
            proposal.proposed_bound > proposal.current_bound,
            "the proposal does not loosen: {} against {}",
            proposal.proposed_bound,
            proposal.current_bound
        );
        assert!((proposal.proposed_bound - 300_000.0).abs() < 1e-9);
        assert_eq!(proposal.evidence.sample, 12);
        assert_eq!(proposal.evidence.regrets, 12);
        assert_eq!(
            platform.open_recalibrations().get(RULE),
            Some(proposal),
            "the proposal is not standing open"
        );
        assert_eq!(
        ...
        // Never applied: the running set is the boot set, and the order the
        // proposal would admit is still refused by the same rule.
        assert_eq!(platform.limits(), &boot, "the running limit set moved");
        refuse_one_over_the_notional_cap(&mut platform);

        // And the same evidence reviewed again is the same proposal, once.
        let (again, problems) = platform.review_rules(start());
        assert!(problems.is_empty(), "{problems:?}");
        assert_eq!(again, None);
        assert_eq!(proposals(&platform).len(), 1);
    }

    #[test]
    fn a_proposal_whose_evidence_evaporates_is_withdrawn_and_cannot_be_enacted() {
        let mut platform = platform();
        regret_twelve(&mut platform);
        ...
        // One signature lands while it stands.
        platform
            .approve_recalibration(RULE, &operator("ops-dana", start()), WHY, start())
            .expect("a first signature on a standing proposal");

        // The window rolls: the same rule's declines now read as correct.
        platform.declined_scores.clear();
        for index in 0..12 {
            platform
                .declined_scores
        ...
        assert!(problems.is_empty(), "{problems:?}");
        assert!(
            summary.as_deref().is_some_and(|s| s.contains("withdrawn")),
            "the review did not report the withdrawal: {summary:?}"
        );
        let written = proposals(&platform);
        assert_eq!(
            written
                ...
                .collect::<Vec<_>>(),
            vec!["proposed", "withdrawn"],
            "{written:?}"
        );
        assert!(platform.open_recalibrations().is_empty());

        // And the countersignature that would have enacted it lands on
        // nothing: not found, not enacted, no artefact.
        let refused = platform
            .approve_recalibration(RULE, &operator("ops-ravi", start()), WHY, start())
            .expect_err("a withdrawn proposal was signed");
        assert!(
            refused.message().contains("proposed no recalibration"),
            "the refusal does not say the proposal is gone: {}",
            refused.message()
        );
        assert!(
            !proposals(&platform).iter().any(|p| p.outcome == "enacted"),
            "a withdrawn proposal was enacted"
        );
    }

    #[test]
    fn enactment_is_refused_without_a_second_different_fresh_signer() {
        let mut platform = platform();
        regret_twelve(&mut platform);
        platform.review_rules(start());
        let boot = platform.limits().clone();

        // A stale credential: authenticated four hours ago.
        let long_ago = start().saturating_sub(Duration::from_hours(4));
        let stale = platform
            .approve_recalibration(RULE, &operator("ops-dana", long_ago), WHY, start())
            .expect_err("a four-hour-old credential signed a recalibration");
        assert!(
            stale.message().contains("re-authenticate"),
            "{}",
            stale.message()
        );
        ...
        let first = platform
            .approve_recalibration(RULE, &operator("ops-dana", start()), WHY, start())
            .expect("a fresh first signature");
        assert_eq!(first.outcome, "awaiting_countersignature");
        assert!(first.artefact.is_none());
        let again = platform
            .approve_recalibration(RULE, &operator("ops-dana", start()), WHY, start())
        ...
        // A first signature older than the window is discarded and named,
        // and the late countersignature starts a new pair rather than
        // completing the old one.
        let much_later = start().saturating_add(Duration::from_days(2));
        let late = platform
            .approve_recalibration(RULE, &operator("ops-ravi", much_later), WHY, much_later)
            .expect_err("a countersignature two days later completed the pair");
        assert!(
            late.message().contains("must follow within"),
            "{}",
            late.message()
        );
        let fresh = platform
            .approve_recalibration(RULE, &operator("ops-ravi", much_later), WHY, much_later)
            .expect("the discarded signature is gone and a new pair starts");
        assert_eq!(fresh.outcome, "awaiting_countersignature");
        assert_eq!(fresh.approver, "ops-ravi");

        // Through all of it nothing was enacted and nothing moved.
        assert!(platform.open_recalibrations().contains_key(RULE));
        assert!(!proposals(&platform).iter().any(|p| p.outcome == "enacted"));
        assert_eq!(platform.limits(), &boot);
    }

    #[test]
    fn enactment_emits_an_artefact_and_leaves_the_running_limits_untouched() {
        let mut platform = platform();
        regret_twelve(&mut platform);
        platform.review_rules(start());
        let boot = platform.limits().clone();
        ...
        assert_eq!(done.outcome, "enacted", "detail: {:?}", done.detail);
        assert_eq!(done.approver, "ops-dana");
        assert_eq!(done.second_approver.as_deref(), Some("ops-ravi"));

        // The artefact: the boot set with exactly one bound replaced.
        let artefact = done
            .artefact
            .as_ref()
            .expect("an enactment carries the artefact");
        assert_eq!(artefact.name, boot.name);
        assert_eq!(artefact.len(), boot.len());
        for (was, is) in boot.limits.iter().zip(&artefact.limits) {
            assert_eq!(was.name, is.name);
            if was.name == RULE {
                assert_eq!(
                    is.kind,
                    qip_risk::limits::LimitKind::MaxOrderNotional {
                        limit: Decimal::from_int(300_000)
                    },
                    "the artefact does not carry the proposed bound"
                );
            } else {
                assert_eq!(
                    was, is,
                    "the artefact moved a limit the proposal was not about"
                );
            }
        }

        // And the running process is exactly as it was: the set it booted
        // with, and the order the artefact would admit still refused.
        assert_eq!(
            platform.limits(),
            &boot,
            "enactment moved the running limit set"
        );
        refuse_one_over_the_notional_cap(&mut platform);
        assert!(platform.open_recalibrations().is_empty());
