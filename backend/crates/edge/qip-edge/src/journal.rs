...
use qip_contracts::reflex::chain_digest_v1;
...
impl Journal {
    ...
    pub fn record(&mut self, decision: Decision, at: Timestamp) -> &JournalEntry {
        ...
        let digest = chain_digest_v1(&previous, sequence, at, &decision);
    ...
    pub fn verify(&self) -> std::result::Result<(), u64> {
        ...
        for entry in &self.entries {
            let expected = chain_digest_v1(&previous, entry.sequence, entry.at, &entry.decision);
