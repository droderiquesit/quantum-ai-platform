  ...
  documentation {
    content   = <<-EOT
      **This is a control working, not a control failing.** The ACT stage
      ...
      It fires on persistence rather than on one cycle, so by the time it
      reaches you sign-off has been withheld somewhere in each of the last two
      half hours, with no clean half hour between them. It is stated in time
      rather than in a number of cycles because the processes that write this
      series cycle at very different rates, and a count would mean something
