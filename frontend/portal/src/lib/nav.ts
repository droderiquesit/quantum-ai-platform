...
export const NAV: readonly NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      {
        // The platform's proposals about its own bounds. Read-only, and the
        // page says why that is not a gap: a proposal is generated by LEARN
        // from regret the platform scored itself and can be supplied by
        // nobody, so a console control that composed one would be a request
        // to loosen a limit wearing a proposal's clothes.
        href: "/risk/recalibrations",
        label: "Rule recalibrations",
        mark: "RC",
        description:
          "Every bound the platform proposed loosening, the regret that argued for it, and what a signature would actually produce",
        reads: ["/risk/recalibrations"],
      },
      {
        href: "/risk/audit",
        label: "Audit trail",
  ...
  },
  {
    ...
    items: [
      ...
      {
        // Why a venue the configuration permits is not being routed to. The
        // withdrawal is the platform's own fail-closed decision and the way
        // back is two signatures at a route this console declares no write
        // against; the page renders the path and the refusal the platform
        // returns for this deployment's credential, and offers no control.
        href: "/execution/withdrawals",
        label: "Venue withdrawals",
        mark: "VW",
        description:
          "Venues the platform withdrew on its own feasibility evidence, the cluster each was withdrawn on, and where the way back stands",
        reads: ["/venues/withdrawals"],
      },
      {
