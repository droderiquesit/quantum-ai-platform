...
/**
 * The password credential, for a deployment that is not behind IAP.
 *
 * Split out of `page.tsx` when the console learned to read Identity-Aware
 * Proxy's asserted identity: whether this card is the right thing to show is
 * a server-side question — it depends on a signature check against Google's
 * published keys — and a client component cannot ask it. `page.tsx` is now a
 * server component that asks, and renders this only when the answer is that
 * nobody has been authenticated upstream.
 *
 ...
 */
export default function SignInCard({ iapConfigured }: { iapConfigured: boolean }) {
  return (
    <Suspense fallback={<p className="text-[12px] text-[color:var(--color-ink-dim)]">Loading…</p>}>
      <SignInForm iapConfigured={iapConfigured} />
    </Suspense>
  );
}

function SignInForm({ iapConfigured }: { iapConfigured: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div data-testid="auth-google-posture">
        {/* Present but disabled: hiding it would make the deployment look
            ...
            says exactly which state this is — and there are two, because a
            deployment behind Identity-Aware Proxy *is* configured for Google
            identity and this request simply did not come through the proxy.
            A single "not configured" caption told that person to wait for an
        ...
        <p className="mt-1 text-[11px] leading-snug text-[color:var(--color-ink-faint)]">
          {iapConfigured
            ? "This console signs in with Google through Identity-Aware Proxy. This request did not arrive through it, so there is no Google identity to continue with. Reach the console at its proxied address."
