...
| `/ledger/users/{user}/eligibility` | POST — operator role. Records an operator's eligibility decision for one user, journalled before the registry adopts it. ~~**Landing in a parallel lane while this was written**; `api_boundary.rs`'s mutating-route set does not yet name it~~ — **stale, corrected 2026-09-06.** It lands: `api_boundary.rs::every_mutating_route_is_reviewed_here_and_each_raises_a_typed_intent` (renamed from `..._is_one_of_five_...` on 2026-09-07, when the investment-request route made the set six; locate it with `grep -n 'fn every_mutating_route' backend/crates/tests/qip-acceptance/tests/api_boundary.rs` rather than by the line number this cell used to give) expects exactly `("Post", "/ledger/users/:user/eligibility")` among them. The console's side was the half that lagged, and it was declared at `1d6ea4c` (`REST.ledgerEligibilityDecide`, `endpoints.ts:87`); until then this write was issued by the panel and refused with a 405 by the console's own gateway |
| `/ledger/users/{user}/investment-requests` | POST — operator role, landed 2026-09-07. §40.9's `investment-api` intent and the only one it has: raises one investment request against a mandate and answers the ledger's verdict — `admitted`, `refused_limit` naming the gate that refused, `detail`, and the user's own `/ledger/users` row beside it. It funds nothing and the body says so with a constant `funded: false`; the typed intent is `Platform::decide_investment`, which journals both outcomes under `kernel/investment` and was the first production caller of `UserLedger::admit`. Contract in `ROUTES-LEDGER.md` |
