...
| → **strategy intent** | **SUBSTITUTED** | **No `Intent` type exists anywhere** — `grep -rn "Intent" services/ edge/ libs/` returns nothing. What runs is `stage_decide` (`platform.rs:2968`) sizing theses into `Proposal`s with legs. Consequently there is **no intent netting, no internal crossing and no contributor vector**, which blueprint §27 makes the mechanism by which many strategies become one net order |
