...
| Trading cell | Compute Engine, in-process risk+OMS+execution, NVMe append-only journal | `qip-edge` cells and the **eight**-stage kernel implement the cell logic in Rust — this row said seven; the `Stage` enum has eight variants and `Stage::all()` returns all eight (`sed -n '/pub enum Stage/,/^}/p' backend/crates/runtime/qip-kernel/src/cycle.rs`), and SENSE, UNDERSTAND, DISCOVER, REASON, SIMULATE, DECIDE, ACT, LEARN is eight names however often it is introduced as seven; the journal is the hash-chained event log. Deployed onto GKE today, not dedicated Compute Engine. |
