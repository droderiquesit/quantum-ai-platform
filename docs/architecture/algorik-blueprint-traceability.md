...
re-scored at `296e187` or at `584c96b` say so inline, and every `path:line`
...
- **[PLANE 6/7 — Execution]** *Ownership:* `qip-edge` (structurally paper-only,
  ...
  *Gaps:* no inventory reservation at the region (F6). Four earlier gaps are
  ...
  on no path (since `584c96b` it installs a desk, from a whitelist nothing
  produces — see the table row); the halt reaches a cell by two wires — the
