...
- **[PLANE 7/7 — Ledger, wallet and treasury]** *Ownership:* `qip-capital`
  ...
  `qip-api/tests/mesh.rs::the_orders_a_cell_reports_reach_the_centres_strategy_books`).
  Re-scored at `e04815e`: what that report's `orders` carry is placements,
  not fills (`cell.rs:3232-3245`), and `settle` books each as a fill
  (`plane.rs:1073`) — the wire gap named in the Plane 6 row, in flight and
  not claimed closed here. *Authority:* records (§39 layer
