...
> below, which is where the rows are. **All nine bands now have a row**, in two
> tables: ADR 0049's rows 1–3 first, then "§48 bands 2, 3, 4, 5, 8 and 9"
> added later the same day. Seven of the nine are scored; **Gate fixtures and
> Path and family simulation are UNVERIFIED on purpose**, each with the missing
> evidence named in its own cell, because a fabricated PARTIAL on a coverage
> requirement nobody has measured is the failure this whole section exists to
> record.
>
...
- **[LAYER 3/7 — Application and API]** *Current:* `qip-api` composes reads and holds no independent financial state. *Keep.* *Change:* none. *Remove:* none. *Change:* **the typed-intent surface (§40.9) is built, 2026-09-07 — this cell said "Defer" and "application APIs still raise no intents, they read", and both are now false.** §40.9 gives `investment-api` exactly one intent and forbids it an order, and `POST /ledger/users/:user/investment-requests` raises it: the body is screened in `qip-api/src/ledger_views.rs`, the user is resolved against the mandate registry, and `Platform::decide_investment` decides it against `UserLedger::admit` under the authenticated operator's identity, journalling both outcomes under `kernel/investment`. It funds nothing — the body carries a constant `funded: false`, and `api_boundary.rs::every_mutating_route_is_reviewed_here_and_each_raises_a_typed_intent` asserts `platform.fund_user(` appears in no source of the crate. What this closes is not only a surface: `UserLedger::admit` had no production caller at all, so the mandate gate was the `MaxExpectedShortfall` shape — proven and unreachable. What is still deferred is narrower and different: per-user *authorisation*. `Principal` carries a bearer-token subject and no `UserId`, so every route on this surface is authorised by role and none can serve a caller their own account. The `Intent` type in `libs/qip-contracts/src/intent.rs` remains the execution vocabulary and is unrelated to this. The gap is the API surface, not the type. *Verification:* `documentation.rs::every_documented_endpoint_exists`; and since `827a40e` the boundary itself is executable — `api_boundary.rs::the_application_layer_depends_on_no_execution_venue_capital_or_edge_crate` from `cargo metadata`, `::the_api_uses_only_the_centre_half_of_the_mesh_and_none_of_its_service_clients` from the sources, with the centre's two signatures pinned by exact expression so a third `.signed(` fails until reviewed.
...
- **[LAYER 5/7 — Data and state]** *Current:* bitemporal records; bounded retention; event log hash-chained; `qip-data-finder` evaluates licensing before use. *Keep.* *Change:* none. *Remove:* none. *Defer:* BigQuery derived series — `qip_storage::gcp::BigQueryWarehouse` is built and constructed nowhere outside its own tests, so this is built-and-unwired rather than absent, and wiring it needs a composition root and a project. **Content-hash manifests are no longer deferred (2026-09-07):** `qip_financial::manifest::SourceManifest` carries source, locator, byte length and the SHA-256 of the document as served, `NewsItem` holds one where it used to hold the article text, and the narrative adapter produces it on the fetch path — blueprint §56.4 rules 34 and 36, which a `NewsItem` copied verbatim into the hash-chained log had been breaking since the record type existed. *Verification:* `absorption.rs`, `resilience.rs`, `truth_loop.rs`, `qip-market-ingestion/tests/narrative_feed.rs::the_article_text_reaches_no_record_the_log_seals_and_a_hashed_reference_stands_in_for_it`.
- **[LAYER 6/7 — Cloud and network]** *Current, re-scored at `296e187`:* the root module wires the blueprint runtime — `catalogue.tf` instantiates `modules/cloudrun` once per deployable in its §46.1 zone with the egress sidecar where a workload carries one, `execution_node` per region from `execution_nodes` (empty in every environment), `trust_zones` binding workloads to zones default-deny both ways, and `egress_proxy` publishing the bootstrap (`808ca32`, `c924191`; seventeen `module` blocks in `main.tf`, count `grep -c '^module "' infrastructure/terraform/main.tf`). The GKE cluster, edge-cell and console-ingress modules, the Helm chart, the raw manifests and the Argo CD stack are deleted (`808ca32`, `67b3e92`, `7d79161`); `deploy.yml` moves each Cloud Run service by digest and fails unless the serving revision carries the attested image (`b85684f`). *What this row said before:* GKE + Argo CD + Kargo + Helm + KEDA as a transitional runtime, to be removed only at ADR 0020 step 5 with recorded approval. *What authorised the change:* the owner's instruction — create the new infrastructure while devouring the old — taken by `808ca32` as approval **for the code and not for an apply**. *Keep:* the wired modules. *Change:* nothing further without a plan. *Remove:* nothing further. *Defer:* the apply, and every observation that depends on one. Re-scored at `e04815e` for §41.4: the node the module boots is configured to run passes — `startup.sh.tftpl:174` writes `QIP_VENUE_FEED=simulated` and the binary's loop runs `Cell::work` on it (`6340610`) — TESTED in `qip-edge-node/tests/pass.rs` and MEASURED nowhere, since `execution_nodes = {}` everywhere. *Verification:* `terraform fmt -check`, `validate` and a plan **NOT RUN — no `terraform` binary exists in this environment**, so every precondition in the new modules is asserted and unexercised; `infrastructure.rs` passed 67 at `808ca32` and `b85684f`, could not pass between `7d79161` and `81dd1cd` because it read the deleted manifests, and passed 59 once `81dd1cd` retargeted it at the runtime that exists — a text scanner's verdict, never a provider's. ~~This layer is CONFIGURED at best and not MEASURED: nothing was applied, and no process has ever been shown to run on either runtime.~~ **That sentence is struck 2026-09-06. Both of its halves are false, and the second is the more misleading.** `dev` has been applied through `infra.yml`'s `up`, repeatedly and with recorded run ids: `PROJECT-PLAN.md` §1's Infrastructure row and its dated log record runs 34 and 35 creating 82 of 83 resources (state went from 163 to 234 objects), run 37 (`33961066585`) creating the fleet-registrar role, its binding and the cluster object — 234 to 237 — and failing after thirty-eight minutes on "only 0 nodes out of 1 have registered", and plan run 38 (`33963633137`) reporting "3 to add, 0 to change, 1 to destroy" against the fix. And processes *have* been shown to run: the same document records direct, unauthenticated `GET`s on 2026-09-04 in which the portal and the landing served `200` with the correct titles and OpenObserve served `200`/`401` as designed. **This row was not re-verified against that record for five re-scores, and the phrase "nothing was applied" was carried forward each time because it was carried forward.** What *is* still true, and is the sentence the struck one should have been: **the apply happened and created no alert policy, which is a different claim.** `grep -n 'module "observability"' infrastructure/terraform/main.tf` returns one line and the block carries no `count` and no `for_each`, so `module.observability` was in that apply; every alert policy inside it is gated on `workload_metrics_exist`, which is `false` everywhere, so every one evaluated to `count = 0`. On 2026-09-06 the three counts in `infrastructure/terraform/modules/observability/main.tf` were **declared 9** (`grep -c '^resource "google_monitoring_alert_policy"'`), **gated 9** (`grep -c 'count *=.*workload_metrics_exist'`) and **edge 2** (`grep -c 'query *= *"[^"]*qip_edge'`); the first two being equal is the load-bearing one. A gate that ran and admitted nothing is evidence about the gate; "nothing was applied" is a claim about the world that was untrue for days. **What remains unmeasured is narrower than this row said:** no execution node exists (`execution_nodes = {}` in all four environments), no collector has been shown to ingest a series, and no authenticated read of a running revision, digest, secret mount or attestation has been made from outside the project. Those, not the apply, are the gap. **Not independently re-observed here:** the `200`s and the run ids are quoted from `PROJECT-PLAN.md`'s record, not re-fetched — this pass has no network to the project and says so rather than implying a fresh observation. **Re-scored 2026-09-06 for §48's "Infrastructure" band only — row 1 of the §48 table below.** The tool that emits every resource named in this bullet is Terraform 1.9.8, not the OpenTofu §48 and rule 77 name; each property the blueprint buys with that tool is met, and the substitution is scored as PARTIAL — TOOL-SUBSTITUTED rather than left implicit in the word "Terraform", which is how a departure from the architecture of record becomes invisible by familiarity. **Nothing else in this bullet was re-verified in that pass** — the apply claim was, separately, and is struck above; everything else here is the `296e187`/`e04815e` score and a reader should treat it as of that date rather than as re-confirmed. **One part of it is known to be stale and is flagged rather than re-scored, because re-scoring it is a larger job than this pass took on:** this bullet says "the Argo CD stack are deleted", and ADR 0036 brought Argo CD and Kargo back on a control-plane cluster. `ls infrastructure/gitops` returns `README.md`, `argocd`, `bootstrap`, `envs`, `kargo`; `modules/cloudrun/main.tf`'s own header says the module "until ADR 0036 also declared the `google_cloud_run_v2_service` itself. It no longer does. The service is a Config Connector `RunService` manifest under `infrastructure/gitops/envs/<env>/`, reconciled by Argo CD". So the runtime this bullet describes is not the runtime in the tree. **UNVERIFIED here** — named so the next reader does not take the bullet at face value, and not given a score this pass did not earn.
- **[LAYER 7/7 — Security, observability, delivery, reliability]** *Current:* three paper layers intact and re-verified by path this pass; two kill-switch wires at the cell since `ff86473` (§46.2), TESTED at the cell and the node in all three release directions since `6a515bb`, the `edge_halted` alert's text naming the polled source since `cd16f79` (locate it with `grep -n polled infrastructure/terraform/modules/observability/main.tf`, which returned one line on 2026-09-06; the line number this cited until then was two off and pointed above the block, and no replacement number is written here because that file is under active edit by more than one lane), and not MEASURED; WIF only; CSI-projected secrets; Binary Authorization; telemetry emitted at the seams. *Keep.* *Change:* none. *Remove:* none. *Defer:* OpenTelemetry spans with cross-plane correlation (§47) — the current surface is a Prometheus-style metric registry, not spans. This row used to say policy-freshness, belief-calibration and reconciliation signals had nothing to emit; at `296e187` all three do — `qip_edge_policy_sequence` and `qip_edge_capability_freshness` at the cell, `qip_belief_brier_score` (`04738ee`) and `qip_central_reconciliation_breaks_total` (`de5d042`) at the centre, the kernel's fourteen newer series spelled in `qip-observability/src/metrics.rs` (`296e187`). What none of them has is a collector that has ingested one. **The second half of this sentence — "or an alert that names one" — is corrected 2026-09-06: one of the four is named by a policy.** `grep -o 'qip_[a-z_]*' infrastructure/terraform/modules/observability/main.tf | sort -u` returned nine distinct series on that date, and `qip_central_reconciliation_breaks_total` is among them, beside `qip_edge_halted`, `qip_edge_reconciliation_breaks_total`, `qip_kill_switch_tripped`, `qip_limit_breaches`, `qip_live_fills_total`, `qip_permission_denials_total`, `qip_proposals_unsigned_total` and `qip_risk_figures_unevaluated`. `qip_edge_policy_sequence`, `qip_edge_capability_freshness` and `qip_belief_brier_score` are named by no policy and that half stands. **This does not make anything observable**, and the distinction is the one the whole layer turns on: the policies are declared, they are gated on `workload_metrics_exist`, and that variable is `false` in every environment, so `dev`'s apply created zero of them — declared 9, gated 9, on 2026-09-06 by the counts quoted in the LAYER 6/7 bullet. A policy that exists in HCL and not in the project pages nobody, which is the same non-result as a policy that was never written; it is corrected here only because "no alert names one" is a claim a reader would check by grepping and find false, and a document caught in a checkable falsehood loses the reader for the true sentences beside it. *Verification:* `security.rs`, `compliance_proof.rs`, `api_boundary.rs`; `egress.rs` and `infrastructure.rs`, both retargeted at `81dd1cd` (14 and 59 passed, per its message) after five commits in which they read manifest paths `7d79161` had deleted. **Delivery re-scored 2026-09-06, and this bullet carried the word "delivery" in its own title while scoring none of it** — rows 2 and 3 of the §48 table below now do: Cloud Build is PARTIAL (it builds the two browser surfaces and not the four Rust images), and Cloud Deploy is MISSING-CURRENT with its guarantee — gradual rollout with automatic rollback on an error rate — **not reproduced by the substitute either**, because nothing in this repository produces an error rate to roll back on. Read that beside the *Defer* clause above: the two are the same gap seen from two directions, and the delivery half is blocked on the observability half rather than on a decision. **Not re-verified in that pass:** the three paper layers, the two kill-switch wires, WIF, CSI-projected secrets and Binary Authorization, all of which are this bullet's earlier score and carry their own dates.
...
**The remaining six of §48's nine bands are scored below, 2026-09-06.** The
blueprint's §48 table has nine: Build and test · Static analysis · Gate
fixtures · Path and family simulation · Attestation · Infrastructure · Deploy —
services · Deploy — node · Admission. ADR 0049 wrote three; the paragraph that
stood here named the other six as UNVERIFIED and said scoring them "needs a
coverage measurement nobody has run". **That was true of one of the six and was
carried over all six.** Four are now scored from the tree with the command in
the cell; two stay UNVERIFIED, and what is missing for each is named precisely
enough that the next reader knows what to run rather than what to guess.

#### §48 bands 2, 3, 4, 5, 8 and 9

| Blueprint element (§48) | Required invariant | Status | Evidence | What is missing |
|---|---|---|---|---|
| §48 "Static analysis" | clippy, cargo-audit, cargo-deny. A new critical advisory blocks the build | **PARTIAL** | Two of the three tools run and block. *clippy* — `.github/workflows/ci.yml` has a `lint` job running `cargo clippy --workspace --all-targets --all-features`, and `grep -n 'RUSTFLAGS' .github/workflows/ci.yml` returns one line, `RUSTFLAGS: "-D warnings"`, set at **workflow level** under the `env:` block, so it reaches that job — the `-D` is not on the command line and a reader checking only the command would conclude warnings are tolerated. *cargo-audit* — the `security-audit` job runs `cargo audit --deny warnings`, which is stricter than the invariant asks: the blueprint blocks on a new **critical** advisory and this blocks on any. *cargo-deny* — **absent, entirely.** `grep -rniE 'cargo.?deny' .github/ scripts/ Makefile backend/Cargo.toml` returned nothing on 2026-09-06 and no `deny.toml` exists at either `deny.toml` or `backend/deny.toml`. The properties `cargo-deny` carries that `cargo-audit` does not — licence policy, duplicate-version bans, source registry allowlisting — are held here instead by `./scripts/check-dependencies.sh` and ADR 0002's two-dependency rule, which is a narrower workspace and a different mechanism, and is why this is PARTIAL rather than a bare gap | A licence and source-provenance check on the *frontend* dependency tree, which is the half neither `check-dependencies.sh` nor `cargo audit` sees. Not scored here and not claimed either way |
| §48 "Gate fixtures" | Every risk-gate and transfer-gate rule, including all path extensions, has a passing and a vetoing fixture. **Coverage of those crates must be total** | **UNVERIFIED — and this is the one band the previous paragraph was right about** | What *is* established: the transfer gate's rules are a closed enum with a total constant, `GateCheck::ALL: [Self; 7]` in `backend/crates/services/qip-capital-fabric/src/gate.rs` — `CorridorAuthority`, `Caps`, `MinimumInterval`, `StatedPurpose`, `SourceBalance`, `VelocityAndAnomaly`, `KillSwitch` — and each of the seven is named in `backend/crates/services/qip-capital-fabric/tests/`, checked by looping the seven variants through `grep -rho "GateCheck::<variant>" tests/`, which returned a non-zero count for every one (9, 6, 1, 3, 1, 2, 6). **That is a weaker fact than the row needs and is stated as such:** a variant *named* in a test file is not a vetoing fixture, and the passing half was not checked at all. The risk-gate crates were not enumerated | The measurement, and it does not exist to be run: `grep -rniE 'llvm-cov|tarpaulin|coverage' .github/workflows/ Makefile scripts/` returned **nothing** on 2026-09-06, so this repository has no coverage instrumentation of any kind and "coverage must be total" has never been measured against anything. Scoring this band needs (a) the risk-gate and transfer-gate crates named as a set, (b) a passing-and-vetoing fixture identified per rule, and (c) a line-coverage run over those crates. None has been done. **Do not let this row become PARTIAL on the strength of the seven-variant grep above** — that is exactly the substitution the `MaxExpectedShortfall` precedent warns about, a count of names standing in for a proof of firing |
| §48 "Path and family simulation" | Each exercised in sim against recorded data before enablement | **UNVERIFIED — blocked on a definition, not on a measurement** | The *family* unit exists and is computed: `qip-optimization-engine`'s `families` module is called in production through `CentralPlane::family_structure` from `stage_learn` (`grep -n 'family_structure' backend/crates/runtime/qip-kernel/src/platform.rs`). But the gate is on **enablement**, and the kernel's own comment at that call site says what forecloses the score: "no seam in this platform consumes a family, and a decision keyed on one would be a gate with no subject." Nothing is enabled per family, so there is nothing for a simulation gate to precede. A strategy-level analogue does exist — `qip-lifecycle`'s `Admission` reads `qip_simulation_engine::validation` and its module doc states the point ("a gate that read a submitted Sharpe" is the failure it prevents) — but a strategy is not §48's unit and scoring it here would be answering a question the blueprint did not ask | Two things, in order. First, **"path" has never been defined in tree terms in this document**, and inventing a mapping would be the same failure as inventing a requirement id — §48's own "Gate fixtures" row says "including all path extensions", so the blueprint treats a path as a first-class unit with its own gate rules, and nothing here says which type that is. Second, the `sim` environment the blueprint's own environment table defines for this ("one node, replay harness, simulated venues with adaptive agents") does not exist: `ls infrastructure/environments/` returns `README.md`, `dev`, `prod`, `stage`, `test` |
| §48 "Attestation" | Signed provenance at build. **Unsigned artifacts cannot deploy** | **PARTIAL** | Signing is wired on every artefact class and is held by named tests. The four Rust images: `deploy.yml`'s image matrix builds, scans and then signs by digest read back from the registry, with `gcloud beta container binauthz attestations sign-and-create` (`grep -n 'sign-and-create' .github/workflows/deploy.yml`), and treats an existing attestation for the same bytes as success rather than an error. The two browser images: `scripts/deploy-frontends.sh` signs each with the pipeline's own attestor and key version (`grep -niE 'binauthz' scripts/deploy-frontends.sh`). Vendored images: `vendor.yml` mirrors with `crane copy` and signs, **per platform manifest and not only the index**, which `qip-acceptance`'s `egress.rs::the_vendor_workflow_attests_every_platform_manifest_and_not_only_the_index` holds — and that test's comment records a real refusal rather than a hypothetical one ("denied by attestor qip-dev-build: No attestations found that were valid and signed by a key trusted by the attestor"), on the linux/amd64 child of an index digest, at the migration's first Cloud Run apply | The second half of the invariant is scored under "Admission" below and is where the gap is, not here. The two named `egress.rs` tests **were re-run on 2026-09-06** — `cargo test -p qip-acceptance --test egress` printed `test result: ok. 23 passed; 0 failed`, with `the_proxy_image_is_pinned_by_digest_and_attested_rather_than_exempted ... ok` and `the_vendor_workflow_attests_every_platform_manifest_and_not_only_the_index ... ok` — so this row's tests are a run, not a citation. What they are is **text scanners over a workflow file**, which is the honest limit: they prove the pipeline still says it signs, never that a signature verified. And no attestation has been read back from the project by anyone outside it, so "signed" is the pipeline's claim about itself everywhere except the one recorded denial |
| §48 "Deploy — node" | Blue-green instance replacement. Shadow mode before taking venue sessions | **PARTIAL — both mechanisms complete in HCL, neither ever exercised** | Blue-green is fully specified rather than gestured at: `modules/execution-node/main.tf`'s `update_policy` sets `type = "PROACTIVE"`, `minimal_action` and `most_disruptive_allowed_action` both `"REPLACE"`, `max_surge_fixed = 1`, `max_unavailable_fixed = 0` and `replacement_method = "SUBSTITUTE"` — the new machine passes its health check before the old one is taken away — with an `auto_healing_policies` block and a 300-second `initial_delay_sec` justified in comment against recreating a booting node. Shadow mode is **structural rather than advisory**, which is the part worth the score: `var.shadow_mode` defaults to `true`, and `google_compute_firewall`'s venue rules are `for_each = var.shadow_mode ? {} : var.venues`, so a node in shadow mode has no venue egress route at all — the file's own words, "This is where shadow mode stops being a claim" | An apply. `grep -rn execution_nodes infrastructure/environments/*/terraform.tfvars` returns `execution_nodes = {}` in `dev`, `prod`, `stage` and `test`, so no managed instance group exists, no replacement has ever run, and no node has ever been observed in shadow mode or let out of it. This is the same defer as LAYER 6/7's, and the band is PARTIAL rather than ALIGNED for that reason alone: the invariant is a property of a deployment and there is no deployment |
| §48 "Admission" | Binary Authorization. **Only attested images run** | **PARTIAL** | The policy is enforcing, not permissive: `modules/binaryauthorization/main.tf`'s `default_admission_rule` sets `evaluation_mode = "REQUIRE_ATTESTATION"`, `enforcement_mode = "ENFORCED_BLOCK_AND_AUDIT_LOG"` and `require_attestations_by` the build attestor, with `global_policy_evaluation_mode = "ENABLE"`. Every Cloud Run surface opts in. The thirteen services declared as Config Connector `RunService` manifests each carry `spec.binaryAuthorization.useDefault: true` — checked by looping every file matching `kind: RunService` under `infrastructure/gitops/envs/` and asserting the flag, which found no declaring file without it (the one file that matched and lacks the flag, `dev/invokers.yaml`, names `RunService` only as an IAM member's target, not as its own kind) — and `qip-acceptance`'s `gitops.rs::every_run_service_holds_the_invariants_its_catalogue_entry_and_the_cloud_run_module_held` holds both halves per service, the opt-in **and** the absence of `breakglassJustification`, which is the field that would bypass the policy for one revision — re-run 2026-09-06, `cargo test -p qip-acceptance --test gitops` printing `test result: ok. 23 passed; 0 failed`. The two browser services take the same path outside GitOps: `scripts/deploy-frontends.sh` passes `--binary-authorization=default` on both `gcloud run deploy` invocations. And the policy has been observed to **refuse a real image**, recorded in `egress.rs` (above) | The execution node, and by nature rather than by omission. `grep -rniE 'binauthz|binary_authorization|attest' infrastructure/terraform/modules/execution-node/` returns exactly one line, a comment saying so: a bare Compute Engine instance has no admission point, "and the substitute is the image build attesting what it packaged". So "only attested images run" holds for every Cloud Run surface and holds for the node only as a build-time convention nothing enforces at boot. Second gap: the citation `deployed-vs-blueprint.md` gives for the Cloud Run opt-in (`modules/cloudrun/main.tf:275-277`) is **stale** — that module declares no service at all since ADR 0036 (`grep -rn 'google_cloud_run_v2_service' infrastructure/terraform/modules/cloudrun/`, which returns only two comment lines), and the flag lives in the manifests instead. The fact survived; the pointer did not |

**Two of these six carry no line numbers into files under active edit, and
that is deliberate.** Each cell names a symbol, a constant or a file, and
carries the command that finds it. The four line-number citations this
document's observability neighbour had to withdraw in a single day are the
argument.

**Where the layer bullets stand after these six.** Static analysis,
Gate fixtures, Path and family simulation and Attestation land in no layer
bullet — they are pipeline and test-suite properties, and §48 does not put them
on the layer map. Deploy — node lands in **[LAYER 6/7 — Cloud and network]**
and Admission in **[LAYER 7/7]**; neither bullet is re-scored by these rows
beyond the corrections already marked in them, and **no other clause in either
bullet was re-verified here.**

Several of the six are also scored in
[`deployed-vs-blueprint.md`](deployed-vs-blueprint.md) against the GKE runtime,
which is a different question from the one this matrix asks; where that
document and this one disagree, this one is the live scorecard and that is
...
`every_mutating_route_is_reviewed_here_and_each_raises_a_typed_intent` names the
...
`the_api_calls_no_platform_mutator_it_has_not_been_allowed`. Both names
carried a count that had already gone stale, and both were renamed to drop it
on 2026-09-07 rather than re-numbered: the set in the test is the
enumeration, and a number in a test's name rots silently while the assertion
beside it stays exact. And while this was being
...
fifth. **It did, and "last" is withdrawn: recounted 2026-09-06, that grep plus
`method: "DELETE"` returns five (`:51`, `:52`, `:53`, `:61`, `:88`), the fifth
being `POST /ledger/users/{user}/eligibility`, declared at `1d6ea4c`.** It had
been issued by `EligibilityPanel` and refused by this console's own gateway for
a wave before anyone added it to the table; the gateway's refusal sentence is
now derived from the table by `describeWrites()` rather than written out by
hand, so the two cannot drift again without `tests/gateway-writes.spec.ts`
saying so. The gateway's allowlist grew a
