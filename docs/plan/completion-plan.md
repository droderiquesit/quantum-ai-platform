...
proposed (`360cfd8`). One defect is open and in flight, filed in §4 as B22.
The checkout at `e04815e` is not clean: a backend agent's uncommitted edits to
seven files under `backend/crates` — the B22 fix — are in the tree as this is
written and do not compile, so the documentation suite for this refresh was
run against a clean export of `e04815e` with these documents copied in.
...
| 7 Ledger, wallet, treasury | capital allocation; envelope; two-signature approval; reservation ledger in the kernel; per-user per-strategy ledger; §43.4 attribution chain at the centre (fill → contributor vector → strategy pro rata); the centre's books settled from venue fills rather than placements; wallet; corridor; transfer gate; destination registry; custody | 5 of 12 | 5/12 | `truth_loop.rs`, `compliance_proof.rs`, `platform.rs::a_second_proposal_is_sized_against_what_the_first_still_holds`; the chain — `qip-kernel/tests/attribution.rs::a_netted_orders_fill_is_attributed_to_its_contributors_with_zero_residual` and `qip-api/tests/mesh.rs::the_orders_a_cell_reports_reach_the_centres_strategy_books` (`7ef6063`, `7d79161`), new since `de5d042`; the books are per strategy and not per user, so the §43.3 ledger stays absent; added absent at `e04815e`: the books are settled from `CellInterval::orders`, which the cell fills from its placements (`qip-edge/src/cell.rs:3232-3245`) and `settle` bills as fills (`central/plane.rs:1073`) — B22, in flight; the rest are Phase 12 and bounded by ADR 0021 |

Summed: **36 of 57 named capabilities at the TESTED bar** (3 + 5 + 0 + 6 +
3 + 14 + 5 over 5 + 7 + 6 + 7 + 5 + 15 + 12), up from 32 of 52 at `584c96b`,
31 of 52 at `296e187` and 25 of 50 at `de5d042`. At `e04815e` five
...
plane, absent and in flight (B22) — so the numerator rose by four and the
...
Summed: **25 of 53 layer items at the TESTED bar** (1 + 3 + 2 + 6 + 5 + 0 +
8 over 7 + 4 + 4 + 8 + 10 + 7 + 13) — the numerator unchanged from 25 of 52
at `584c96b` and the denominator one wider, because layer 5 gained the item
B22 names, absent; up from 23 at `296e187` — which this paragraph then wrote
...
| B22 | **Bill the wire's fills, not its placements** — open and in flight. Since `cb79b46` the cell books a fill only when the venue reports one, but the uplink's `CellInterval::orders` (`qip-mesh/src/delta.rs:182`) is still built from the pass report's *placed* orders (`qip-edge/src/cell.rs:3232-3245`), and `CentralPlane::settle` (`central/plane.rs:1068`, the loop at `:1073`) books every one of them as a fill into the strategy books and, through B20, into the risk aggregate — so a resting order the venue never filled is a position and an exposure at the centre. Two claims about one fact, and the centre's is the wrong one; invisible while no node ran passes, and the first thing a running one would produce. A backend agent's uncommitted edits to `cell.rs`, `mesh.rs`, `wire.rs`, `delta.rs`, `plane.rs`, `central/mod.rs` and `metrics.rs` are in the tree as this row is written; nothing here claims they land | §43.4, rule 12, principle 6 | P3 | B11, B20 | None; in flight | The delta carrying the fills the venue reported, distinct from placements; `settle` reading those; a test in which a rested, unfilled order reaches the centre and moves no book and no aggregate; B20's test still passing | 1, in flight |
...
concentration decision, behind D13 and ADR 0027. **Unblocked today:** B8,
B10's central half, B12, and B22 once the in-flight change lands or is
abandoned; B13's owner half and B23 wait on the owner.
...
**Alignment-done.** Closer than at `584c96b`, and for the first time the
distance is a defect rather than an absence. Of the seven alignment items,
five are spent; A6 still waits on an attested sidecar digest nobody has
pinned; and A5's desk half has closed on its own terms — the whitelist is
produced and shipped, the node runs passes — leaving only the normaliser's
disposition to the owner. What changed the shape is this: the edge plane's
controls, which every earlier version of this paragraph had to describe as
proven by the cell's tests and reached by no deployed process, are now in the
deployed binary's loop (`6340610`), and the first thing a running node would
ship — its placements, as `orders` — the centre bills as fills (B22). That is
an alignment-done failure by §1(a)'s own first clause: two claims about one
fact, one of them wrong, and a control — B20's exposure charge — that fires
on something that did not happen. It is in flight and not closed here. The
boundaries are still enforced structurally, the application layer's reach is
still a test rather than a reading, the paper-trading line still has three
layers and a test on each, and the venue feed is a fourth refusal in the same
register — `QIP_VENUE_FEED` accepts one value and names ADR 0003 for any
other. The cloud layer is unchanged: the runtime in the tree is the
blueprint's, never seen by a Terraform binary, never planned, never applied,
and no process has ever been shown to run on it. Call it two slices from
aligned — B22 landing with its test, and the owner's sentence on the
normaliser — with one layer that cannot be scored above zero from this
environment, and a full gate that was running, not read, when this was
written.
...
between here and the first gate is still an egress path — a sidecar written
in Terraform that no binary here can plan, pointed at no vendor, in a project
nothing can reach; after that, seven days of streaming nobody can run from
this environment; after that, the Phase 2 gate, which the blueprint expects
to fail more often than pass. One new thing sits ahead of any deployed desk
trading at all: the default concentration cap refuses the first order into
the catalogue every root now loads, until ADR 0027 is decided (B23). Phases 0
to 3 are twenty-three rows, eleven spent; of the twelve open, four are
unblocked today and the rest wait on a Terraform binary, a vendor host, an
ADR, the crossing interval, or the owner. Phases 4 to 19 are well over a
