...
So **0 of 7 planes are ALIGNED, 6 of 7 are PARTIAL, 1 of 7 is MISSING.**
**That MISSING is Valuation, and it is the matrix's status and no longer this
document's reading** — the row below scores the plane 5 of 6 as of 2026-09-07,
five of §16.1's six engines having a production caller. The status table is
reproduced as the matrix wrote it rather than silently updated, because
`gap-matrix.md` is the matrix's own document to re-trace; what a reader must
not do is take the MISSING here and the 5/6 below as this file contradicting
itself. It is a stale row in another register, named. A
...
|---|---|---|---|---|
| 1 Ingestion | absorb records; entity resolution; licensing before use; one live source sustained; deep-web tier | 4 of 5 | 4/5 | **Re-verified 2026-09-07 and deliberately unmoved. `f3db9ab` hardened two capabilities this row already counts and added no sixth, and the temptation to score it is exactly what this row must refuse.** What landed: `ConnectorFeed::journal_to` and `StandingAdmission` reached both composition roots — `grep -rn 'journal_to\|StandingAdmission' backend/crates/apps/*/src/main.rs backend/crates/apps/*/src/feed.rs` finds `feed.journal_to(storage.key_value(StreamJournal::NAMESPACE)?)` in `qip-api/src/main.rs` and `qip-fastbrain/src/main.rs` (both above any `#[cfg(test)]` marker in their files; `grep -n 'cfg(test)' backend/crates/apps/qip-api/src/main.rs backend/crates/apps/qip-fastbrain/src/main.rs`) and `StandingAdmission::open`/`::over` in each root's `feed.rs`. Before it, every restart began with an empty dedup window against a connector that re-decodes the whole table each poll, so the whole rates table was republished as new observations at every rollout, and the licensing gate was consulted once per process, so a licence expiring on day three of a seven-day run kept granting for four more. **Both defects are in the *sustained* capability, and neither closure is evidence for it.** "One live source sustained" needs a deployed process fetching through a deployed egress proxy for a period; `f3db9ab`'s own message says the wirings are "proven in-session against a loopback server" and that "a ledger reading `sessions: 1` is what an in-session proof produces". A restart-durable dedup window is what makes a sustained run *measurable*; it is not a sustained run. The row moves when a deployment streams, not before. `absorption.rs`, `sense.rs`, `qip-fastbrain/src/licensing.rs`; live source is PARTIAL (one tick, no deployment) and is the only thing keeping this row below 5. **Corrected 2026-09-06, up from 3 of 5:** this cell said "deep-web tier MISSING" and the tier model has existed since `4efd34d`. `ls backend/crates/services/qip-data-finder/src/tier.rs` finds it; `grep -c '#\[test\]' backend/crates/services/qip-data-finder/tests/tiers.rs` printed `16`; `grep -n tier backend/crates/services/qip-data-finder/src/finder.rs` finds the `use crate::tier::{…}` and the assessment that asks the tier "before any request is made". `PROJECT-PLAN.md` §1's Ingestion row has read 4/5 since that commit and this row did not follow it |
| 2 Cognition | world model; causal graph; episodic memory; hypotheses; belief formed, graded and read at sizing; counterfactuals with a production caller; self-model | 7 of 7 | 7/7 | **Re-scored 2026-09-07, up from 6 of 7, and the move is a re-wording of the item rather than new code — read the re-wording before the number.** The item read "belief stage in the cycle" and counted a ninth `Stage` arm. **There is no `Belief` stage and there is not going to be one**: `sed -n '/pub enum Stage/,/^}/p' backend/crates/runtime/qip-kernel/src/cycle.rs` returns eight arms — `Sense`, `Understand`, `Discover`, `Reason`, `Simulate`, `Decide`, `Act`, `Learn` — and CLAUDE.md declares the eight settled, naming the count as the first thing every agent reads. So the row was scoring the platform against an architecture it deliberately does not have, and would have stayed at 6 of 7 forever while the capability underneath it was complete. The capability is belief as a thing the platform **forms, grades and sizes against**, and all three seams have production callers, each verified here by walking from the stage rather than by an existence check: **formed in REASON** — `ReasoningEngine::reason` marks `self.beliefs.absorbed(input.now)` (`grep -n 'beliefs\.absorbed' backend/crates/services/qip-reasoning-engine/src/engine.rs`, one hit, inside `pub fn reason`), reached from `Platform::synthesise` → `reason_about_the_queue` → `stage_reason` (`grep -n 'fn stage_reason\|fn reason_about_the_queue\|fn synthesise\|self.reasoning.reason' backend/crates/runtime/qip-kernel/src/platform.rs`, and read the enclosing function rather than the line); **graded in LEARN** — `calibrate_resolved`, called from `stage_learn` (`grep -n 'calibrate_resolved\|fn stage_learn' …/platform.rs`; `04738ee`, `learning.rs::a_cycle_that_resolves_a_thesis_grades_it_and_moves_the_calibration_series`); **read at sizing** — `BeliefFreshness::assess(self.reasoning.beliefs().last_updated(), BELIEF_HORIZON, now)` inside `Platform::central_degradation`, whose caller is `construct_from`, the sizing path (`grep -n 'BeliefFreshness::assess\|central_degradation\|fn construct_from' …/platform.rs`), so a stale belief narrows the central multiplier rather than being ignored. **Record the absence as deliberate, not as a gap:** a `Belief` stage would be a ninth arm nothing asked for, and forming a belief inside REASON is where the panel that forms it already runs. `PROJECT-PLAN.md` §1 has read 7/7 throughout, counting the grading; it was the closer of the two readings and this row now counts the same three seams it does, so the two documents no longer differ here. **7 of 7 is this table's TESTED bar — built, wired to a caller, tested — and not a claim that any of it runs**: nothing in this repository is deployed. Prior text of this cell, kept because the trend is the useful part: **corrected 2026-09-06, up from 5 of 7: this cell said "no self-model" and one is composed and called in production.** `grep -n 'SelfModel\|self_model' backend/crates/runtime/qip-kernel/src/platform.rs` finds the import from `qip_learning_engine::self_model`, the field on `Platform`, its construction, `self.self_model.absorb(evaluation, &components)` and `self.self_model.origin_factors()` feeding `set_origin_factors` — a caller, not a registration. `understanding.rs`, `reasoning.rs`, `world.rs:41`, `causal.rs:234`; belief still SUBSTITUTED (flow 2) though now graded — `learning.rs::a_cycle_that_resolves_a_thesis_grades_it_and_moves_the_calibration_series` (`04738ee`); counterfactuals gained their caller — `learning.rs::a_refused_order_is_priced_once_its_horizon_has_passed_and_charged_to_its_gate` (`b9e2242`), up from 4 of 7. **The sentence that stood here — "the one that keeps this row at 6 of 7 is 'belief stage in the cycle', and it is where this table and `PROJECT-PLAN.md` §1 (which reads 7/7) genuinely disagree rather than one being stale" — is withdrawn on 2026-09-07.** It was accurate about the two documents and wrong about what was being measured: it settled for describing a disagreement instead of resolving it, and the resolution was available the whole time, because the stage enum it quoted is itself the evidence that the item was mis-worded. A disagreement two documents can both cite is not a stalemate when one side is counting something the architecture refuses to have |
| 3 Valuation | six engines (§16.1) | 5 of 6 | 5/6 | **Re-scored 2026-09-07, up from 4 of 6: corporate actions gained a production caller at `367bb92`, on this row's own bar — a production caller, not an existence.** `qip_market::corporate_action` computed adjustment factors and `Position::apply_adjustment` had no caller outside a test, so a split crossed the tape and the bar series kept its pre-split closes. `Platform::stage_sense` now calls `apply_due_corporate_actions`, which calls `apply_corporate_action`. Verify with `grep -n 'apply_due_corporate_actions\|fn apply_corporate_action\|fn stage_sense' backend/crates/runtime/qip-kernel/src/platform.rs` and read the enclosing function rather than trusting a line number: on 2026-09-07 the call sat six lines inside `fn stage_sense`, and `apply_due_corporate_actions` and `apply_corporate_action` are both defined above it. TESTED by `mod corporate_action_tests` in the same file (`grep -n 'mod corporate_action_tests' …/platform.rs`), five tests that drive `Platform::run_cycle` rather than the private helpers — a split the tape has crossed, a split announced ahead of its ex-date, the same split absorbed twice, a cash dividend, and an unpriceable ratio. **The sixth is the volatility surface, and it is BLOCKED rather than absent — score it that way.** `grep -rn 'VolatilitySurface' backend/crates --include=*.rs` outside `qip-market` returns two doc comments (`qip-market/src/curve.rs`, `qip-kernel/src/valuation.rs`) and one re-export; a doc comment is not a caller. The blocker is ADR 0050 (`docs/adr/0050-what-an-option-quote-source-must-satisfy-before-the-volatility-surface-has-a-caller.md`, status *proposed*), an owner-side licensing evaluation of an option-quote source, outside this repository. ADR 0050 refuses the shortcut by name — an engine fed a number the platform invented reads as a capability and is not one — so nothing in this tree can move this item, and a later re-score should not read it as neglected work. **5 of 6 is built-wired-and-tested and is not a claim that anything runs**; nothing here is deployed. Prior text of this cell: **corrected 2026-09-06, up from 0 of 6, and this is the largest single error §2.2 carried.** The cell said "no engine prices anything"; four of §16.1's six are wired into the kernel and reachable by command. `grep -rn 'CreditRegister::from_universe\|CreditProfile::from_object\|IlliquidValuator::mark_object\|CommitmentBook\|VolatilitySurface' backend/crates/runtime/qip-kernel/src/` finds *term structure* (`CreditRegister::from_universe` called from `platform.rs`), *credit* (`CreditProfile::from_object` called from `valuation.rs`), *illiquid valuation* (`IlliquidValuator::mark_object` called from `platform.rs`) and *cashflow and commitments* (`CommitmentBook` held as a field on `Platform` and constructed there) — and finds the *volatility surface* only inside a doc comment, which is not a caller. **The clause that followed — "*corporate actions* remain absorbed as a world event and not priced. So 4 of §16.1's six" — was true when written and stopped being true at `367bb92`; it is superseded by the paragraph opening this cell.** [`rescore-2026-09-06.md`](rescore-2026-09-06.md) (committed at `74ef5bd`) established this and also established that **this row's denominator was the wrong six** — it had substituted the liquidity ladder, which §16.1 does not name, for corporate actions, which it does; the ladder is genuinely wired and is a seventh thing, counted nowhere here. `PROJECT-PLAN.md` §1 already tabulates this row's old 0/6 as one of four defensible readings (the 70.42% line); both documents read the same figure — 4/6 from 2026-09-06 and **5/6 from 2026-09-07**, corrected in the same pass in both — and the "MISSING-CURRENT, as `completion-plan.md` still scores the plane" row in that table is now describing a state this file has left |
| 4 Intelligence | statistical gate; champion/challenger; drift detection; training; corridor policy; cumulative trial accounting across runs; holdout band as an output of validation | 7 of 7 | 7/7 | `lifecycle.rs`, `evolution.rs`, `training.rs`, `qip-deepbrain/src/learning.rs:279`; the band — `lifecycle.rs::a_holdout_admission_carries_the_band_its_validation_produced` (`d0558b4`), new since `de5d042`; **corrected 2026-09-07, up from 6 of 7: this cell read "corridor policy has no subject (Phase 12)" and the subject landed at `27da0c5`.** `qip-lifecycle`'s `corridor.rs` derives a standing from the weakest rung any strategy a corridor funds is standing on, and `Platform::corridor_funding` is what `Platform::decide_fabric` re-derives and compares a gate command's stated ruling against, so a corridor cannot be assessed against a figure the layer that sets corridor policy never produced — `qip-kernel/tests/ledger.rs::a_corridor_funding_a_strategy_that_holds_no_capital_is_suspended_and_the_gate_vetoes_it`. Locate it with `grep -n 'corridor_funding' backend/crates/runtime/qip-kernel/src/platform.rs` rather than trusting a line number. **The limit rides with the promotion rather than being dropped in it, and it is the same shape row 7 records for four of its five fabric capabilities:** `decide_fabric` has exactly one production caller — `reconcile_wallet`, from `stage_learn` — and it passes only `FabricCommand::Wallet`; `grep -rn 'declare_corridors\|decide_fabric' backend/crates --include=*.rs` finds the `Gate` arm and the policy declaration in `qip-kernel/tests/ledger.rs` and nowhere else. So in a deployment no corridor policy is declared and no gate command is issued, and the control is proven and unreached. 7 of 7 is this table's TESTED bar — a named passing test — and it is not a claim that anything runs; cumulative trial accounting is TESTED at the crate (`::a_second_run_is_corrected_against_the_first_runs_trials_as_well`, `9332bcb`) and enrolled by the factory (`94dd7e2`) and, since `aa66c5d`, opened durable in every root (`qip-kernel/tests/trial_book.rs::a_book_reopened_from_the_same_store_carries_the_familys_lifetime_count_forward`) with the quarter budgeted at `e31aae4` — counted present at `584c96b`, up from absent at `296e187`, where the deployed book was in-memory |
| 5 Optimisation | routing gate; classical baseline every time; authority boundary structural; family clustering; multi-horizon reconciliation | 5 of 5 | 5/5 | `optimization.rs`, `architecture.rs` solver tests, ADR 0006. **Corrected 2026-09-06, up from 3 of 5:** family clustering moved from absent to having a production caller at `76c677f` and this row did not follow it. `grep -n 'family_structure' backend/crates/runtime/qip-kernel/src/platform.rs` finds `self.central.family_structure(now)` and `awk` over the same file puts that call inside `fn stage_learn`, whose own comment states the case ("the family clustering was a complete, refusing, tested stage with no caller … `central::realised` now retains the grant on a day that settled nothing, so it can"). Multi-horizon reconciliation kept it below 5 until 2026-09-07 and no longer does. `PROJECT-PLAN.md` §1 corrected the same figure at `4ac6027` and the two now agree. **Corrected 2026-09-07 to 5 of 5, on the same bar this row used to promote family clustering — a production caller, not an existence.** `Platform::stage_learn` calls `Platform::arm_horizon_gate`, which arms the §23.4 gate over the lifecycle ledger from figures the platform measured that cycle (`CommitmentBook::unfunded_total` and `CapitalCoordinator::drawdown`, not configured constants), and a promotion to a capital-holding rung whose bucket is over its pool is refused by name through `HorizonAssurance::admit` → `PoolReconciler::fund` → `qip_optimization_engine::horizons::reconcile`. Locate it with `grep -n 'arm_horizon_gate' backend/crates/runtime/qip-kernel/src/platform.rs` rather than trusting a line number here. **Two limits, stated so that 5 of 5 is not read as "operating in a deployment", which it is not.** First, the two facts nobody measures — the four-way split, and which horizon each strategy sits at — are `CentralConfig::horizons`, and no composition root loads it, because configuration is read in `backend/crates/apps/**` and that was outside the lane's paths; with none stated the plane arms nothing and records that it armed nothing, deliberately rather than arming an empty reconciler that would refuse every promotion, which is the `MaxExpectedShortfall` shape this project keeps finding. Second, nothing in this tree promotes a strategy outside a test — `grep -rn '\.promote(' backend/crates/` finds only `ModelRegistry::promote` in production — so the gate is armed and reachable while the event it refuses has no producer. That second gap predates the wiring and was not invented to excuse it. The capability is built, wired to a stage, and tested (`qip-kernel/tests/central.rs`, five tests, seven mutations fired); it is not yet exercised by a deployment, and neither is anything else here. **The other four re-verified from code on 2026-09-07, each by its production caller rather than by its existence:** routing gate — `self.cost_router.select(&context)` in `platform.rs` (`grep -n 'cost_router.select\|cost_router.assess' backend/crates/runtime/qip-kernel/src/platform.rs`); authority boundary structural — `Determinism::Required => Ok(Routing::Deterministic(...))` in `qip-cost-router/src/router.rs`, and `Routing::model_tier` returns `None` for that arm because the variant has no field to read it from, which is the structural half; family clustering — `self.central.family_structure(now)` inside `stage_learn`, its result kept on `cycle_family_structure` and written to the journal entry, so it is not a computed value dropped; classical baseline every time — `ComputeRouter::solve` solves the baseline unconditionally and `PortfolioConstructor::construct` calls it at `construction.rs:256`, reached from `stage_decide`. **One gap was found inside that last item and closed:** the `RoutingDecision` carrying the comparison — chosen solver, baseline objective, improvement, quantum note — was read by exactly one test in `qip-portfolio-engine` and by nothing else in the workspace, so the control ran on every cycle and the platform kept no evidence that it had. It is now on `CycleJournalEntry::solver_routing`, taken (not cloned) at the seal so a cycle that reaches no construction cannot inherit the last one's, and proven by `qip-kernel/tests/solver_routing.rs` (2 tests, 4 mutations). The score is unchanged: the baseline was always computed, and what changed is that it is now reproducible from the event log |
| 6 Execution | paper-only cell; envelope admission; intent netting; internal crossing; contributor vector on the uplink; halt reaching a cell; §6.2 narrowing; feasibility gate; per-region reservation; crossing settled to books; leg producer for cycles; the node running passes; a fill as a venue fact; a stated pricing policy; the cycle whitelist produced and shipped | 15 of 15 | 15/15 | **Corrected 2026-09-06, up from 14 of 15:** this row said "Reservation still CONTRADICTS (F6)", and the region table has been consulted at the cell since `cac99d3`. `grep -n 'RegionTable' backend/crates/edge/qip-edge/src/cell.rs` finds the import, the `region_allocation` field and `with_region_table`; `backend/crates/edge/qip-edge/tests/region_table.rs` is a whole test file driving it. The trailing "Reservation still CONTRADICTS (F6)" sentence below is left in place as the record of what this row said and is **superseded by this sentence**; F6 itself is the flow table's to re-trace and was not re-traced here. `qip-edge/tests/cell.rs`, `qip-edge-node/tests/gateway.rs`, `qip-api/tests/mesh.rs::a_cycle_ships_a_signed_payload_the_cell_verifies_and_a_trip_reaches_it`; three moved since `de5d042`: feasibility — `feasibility.rs::an_off_lot_intent_is_refused_before_netting_and_never_rides_a_feasible_strategys_order` (`95a4932`); crosses settled at the centre's books — `qip-kernel/tests/attribution.rs::an_internal_cross_moves_both_contributors_books_at_the_mid_and_the_close_out_is_exact` (`7ef6063`); the leg producer — `arbitrage.rs::a_cycle_on_the_cells_own_books_becomes_its_legs_as_orders_in_one_pass` (`71f9465`). Reservation still CONTRADICTS (F6). Four added at `e04815e`, all TESTED: the node running passes — `qip-edge-node/tests/pass.rs::a_node_with_the_simulated_feed_runs_a_pass_and_the_pass_time_series_move`, `::a_venue_feed_other_than_the_simulator_is_refused_at_start_naming_adr_0003` (`6340610`); a fill as a venue fact — `qip-edge/tests/fills.rs::an_order_the_venue_accepted_is_not_a_fill_until_the_order_entry_channel_reports_one` and `pass.rs::a_resting_order_the_venue_fills_on_a_later_pass_is_confirmed_and_the_node_keeps_trading` (`cb79b46`, `b8d18d3`); a stated pricing policy — `qip-edge/tests/pricing.rs::an_intent_with_no_stated_pricing_is_refused_and_nothing_reaches_the_venue`, `::a_resting_order_rests_at_the_mid_and_is_withdrawn_when_its_time_to_live_elapses` (`383d4e7`); the whitelist — `qip-kernel/tests/central.rs::issuing_a_whitelist_through_the_platform_journals_what_was_issued`, `qip-api/tests/mesh.rs::a_cycle_ships_the_desk_a_live_grant_funds_as_a_whitelist_the_cell_verifies` (`5396679`, `91d20f5`). The caveat that weighed on the first ten until `e04815e` — that `qip-edge-node` called `Cell::work` on no path — is gone; the one that remains is that no node is deployed, so TESTED never means a measured pass |
| 7 Ledger, wallet, treasury | capital allocation; envelope; two-signature approval; reservation ledger in the kernel; per-user per-strategy ledger; §43.4 attribution chain at the centre (fill → contributor vector → strategy pro rata); the centre's books settled from venue fills rather than placements; wallet; corridor; transfer gate; destination registry; **custody as an enforced boundary** | 11 of 12 | 11/12 | **Re-scored 2026-09-07, DOWN from 12 of 12, and this is a correction of the item's name rather than a regression in the tree — nothing was un-built.** This row's twelfth read "custody" and counted the custody *table* — `qip-capital-fabric/src/custody.rs`, a policy record, TESTED at the crate in `tests/corridor_and_gate.rs`. `PROJECT-PLAN.md` §1's twelfth reads "custody as an **enforced boundary** rather than a policy record" and scores it absent. **The two documents were not counting different twelves; they were counting the same slot under two different meanings of one word, and the meanings differ by exactly the thing that makes it a control.** The module states its own limit — `sed -n '1,13p' backend/crates/services/qip-capital-fabric/src/custody.rs` reads "ADR 0021 permits that table and refuses the machinery the blueprint puts behind it — the policy *engine* that holds a share of a signing key and releases it on approval. So this module holds the table and nothing else". A table nothing enforces is the `MaxExpectedShortfall` shape this project keeps finding, and scoring it present is how a reader concludes custody is held. So the item is renamed to the enforced boundary and scored absent in both documents. **It is REFUSED, not merely open, and refused with no expiry**: ADR 0021's row is "MPC signing corridors, withdrawal APIs, live venue submission — Refused. No exception, no flag, no test-only shortcut", liftable only by a recorded owner decision to change the platform's purpose. It cannot move without that, and no work in this repository closes it. **The cost of leaving this uncorrected was concrete and is the reason it is being paid now:** this row's 12 and §1's 11 differed in one direction while Cognition's 6 and §1's 7 differed in the other, so the two documents summed to the *same* total by different memberships, and a reader who checked the arithmetic found it correct in both and concluded they agreed. Both differences are closed in the same pass: Cognition upward on a re-worded item, Ledger downward on a re-worded item, and the totals now agree because the memberships do. Prior text of this cell, kept because the reasoning about the bar is still the reasoning: **corrected 2026-09-06, up from 6 of 12, and the correction is about this table's own bar rather than about new code.** The five this row called absent — wallet, corridor, transfer gate, destination registry, custody — are all TESTED at the crate, which is the bar §2.2 declares ("a capability counts as present only at the TESTED bar", and `PROJECT-PLAN.md` §1's preamble makes TESTED mean a repository test rather than a deployed one). `ls backend/crates/services/qip-capital-fabric/tests/` returns `capital_fabric.rs`, `corridor_and_gate.rs`, `journal.rs`, `wallet.rs`, and `grep -n 'custody::\|destination::\|DestinationRegistry' backend/crates/services/qip-capital-fabric/tests/corridor_and_gate.rs` finds the custody table and the destination registry driven there. **Two things this must not be read as saying.** First, wallet is the only one of the five with a production caller: `grep -n 'qip_capital_fabric' backend/crates/runtime/qip-kernel/src/platform.rs` imports `wallet`, `journal` and the forecasting and transfer-cost types and imports **no** `gate`, `corridor`, `destination` or `custody` — so four of the five are proven and unreached, exactly the `MaxExpectedShortfall` shape, and they are counted here only because this table's stated bar counts them. Second, this row said until 2026-09-06 that the one still absent was the **per-user per-strategy ledger** (§43.3), "the books are per strategy and not per user", and **that was false when written** — `0599092` landed the pair-keyed book on 2026-09-05 and this row was edited afterwards at `bec55cc` without the clause being re-checked, which is citation rot of the kind `.claude/rules/domains/observability.md` documents. Checked from code rather than from either plan: `pub type LedgerKey = (UserId, StrategyId)` (`qip-capital/src/ledger/book.rs:79`) keys `books: BTreeMap<LedgerKey, StrategyBook>` (`:160`), so a user-and-strategy pair is the key and not two aggregates; `qip-capital/tests/user_ledger.rs::the_books_report_in_ledger_key_order_however_they_were_funded_and_each_book_reports_its_own_balance` funds two users across two strategies and asserts **four** distinct books, which two aggregates could not produce; the production caller is `qip-api/src/mesh.rs:1387` (`self.platform.ingest_cell_report(...)`, outside any `cfg(test)`) reaching `UserLedger::journal_pro_rata`; and `qip-kernel/tests/ledger.rs::a_fill_across_two_entitled_users_books_two_shares_that_sum_to_the_fill_with_the_remainder_recorded` proves a fill lands in the right cell at the zero-residual bar. **That correction moved the per-user per-strategy ledger from absent to present and stands; what has changed since is only the twelfth item's name, so the row is 11 of 12 rather than 12 of 12. The true remaining limit on the eleven that are present is narrower than the one this cell used to assert and is stated here instead of it:** the pair-keyed books carry attributed cash and entry counts (`StrategyBook` holds `BTreeMap<Currency, CashBalance>`, `entries`, `last_entry_at`) and **not position quantity**, because the attribution record carries neither quantity nor price. Per-pair PnL is present; per-pair position is not. **The sentence that stood here — "`PROJECT-PLAN.md` §1 reads 11/12 against a different enumeration whose twelfth is custody-as-an-enforced-boundary … they are counting different twelves and must not be added together" — is withdrawn 2026-09-07.** They were not different twelves. Eleven of the twelve names matched exactly and the twelfth was one word carrying two meanings, which is a resolvable disagreement that had been written up as an unresolvable one; describing it accurately let it survive another re-score. Both rows now read 11/12 over the same enumeration. `truth_loop.rs`, `compliance_proof.rs`, `platform.rs::a_second_proposal_is_sized_against_what_the_first_still_holds`; the chain — `qip-kernel/tests/attribution.rs::a_netted_orders_fill_is_attributed_to_its_contributors_with_zero_residual` and `qip-api/tests/mesh.rs::the_fills_a_cell_reports_reach_the_centres_strategy_books` (`7ef6063`, `7d79161`; the second renamed from `the_orders_...` at `d59505d`, when what it proves reaching the books became the fill), new since `de5d042`; the §43.3 per-user per-strategy ledger is TESTED and production-reached — `qip-capital/src/ledger/book.rs:79,160`, `qip-api/src/mesh.rs:1387`, `qip-kernel/tests/ledger.rs::a_fill_across_two_entitled_users_books_two_shares_that_sum_to_the_fill_with_the_remainder_recorded` — and the "per strategy and not per user" clause this cell used to carry was false from `0599092` onward; added absent at `e04815e` and TESTED at `5290bb9`: the books are settled from the delta's `fills` (`qip-contracts/src/wire.rs:93`; `qip-edge/src/mesh.rs:214`) and never from its `orders`, which `settle` registers as sent (`central/plane.rs:1161`, `:1187`) — `qip-kernel/tests/risk_aggregates.rs::a_sent_order_the_venue_has_not_filled_charges_nothing_to_the_aggregate`, `::the_same_order_filled_in_the_next_report_charges_exactly_the_fill`, `qip-api/tests/mesh.rs::an_order_a_cell_reports_sent_and_unfilled_reaches_no_book_and_charges_nothing` (B22, struck); the rest are Phase 12 and bounded by ADR 0021 |

Summed: **54 of 57 named capabilities at the TESTED bar** (4 + 7 + 5 + 7 +
5 + 15 + 11 over 5 + 7 + 6 + 7 + 5 + 15 + 12 — both sums verified by
addition), **re-added from the rows above on 2026-09-07 rather than carried.**

**Read this before quoting 54.** The bar is *built, wired to a caller, and
tested*. It is not *running*. **Nothing in this repository is deployed**, and
the number cannot be read as a working platform: `grep -rn execution_nodes
infrastructure/environments/*/terraform.tfvars` returns `{}` in dev, test,
stage and prod, so no edge cell exists anywhere;
`workload_metrics_exist` defaults to `false` in
`infrastructure/terraform/variables.tf` and is commented out in every
environment's tfvars, so every alert policy is absent and a reconciliation
break on either plane pages nobody; and no Cloud Run service has a metrics
collector attached, refused rather than pending on an upstream image three
advisories behind. Fifty-four capabilities that a `cargo test` run exercises
and no deployment has ever executed is what this figure says. A reader who
takes it for an operating system has been misled by the number and not by the
rows, which is why the limit is attached to the sum rather than left in them.

**Three terms moved on 2026-09-07 and two of the three moved because an item
was mis-named, not because code landed.** Cognition 6 → 7 (the item read
"belief stage in the cycle" and counted a ninth `Stage` arm the architecture
deliberately lacks; re-worded to the capability, which has three production
callers). Ledger 12 → 11 (the twelfth read "custody" and counted a policy
table; re-worded to "custody as an enforced boundary", which ADR 0021 refuses
with no expiry). Valuation 4 → 5, and that one *is* code: corporate actions
gained a production caller in `stage_sense` at `367bb92`. The two re-wordings
cancel in the total and are recorded separately anyway, because a net-zero
pair of corrections is precisely what a sum hides.
This line read "50 of 57" over the terms `4 + 6 + 4 + 6 + 4 + 15 + 11` while
the paragraph twenty lines below it read 52 and the rows themselves read 52 —
three of its terms (Intelligence, Optimisation, Ledger) were the figures their
rows had *before* `27da0c5`, `d8edaf7` and the 2026-09-06 Ledger correction. A
sum written as digits beside the terms it came from is still only as fresh as
the last person who re-added it, which is why the terms are spelled out: the
arithmetic is checkable without opening the table, and a term that disagrees
with its row is then visible rather than absorbed into a total. Intelligence
moved to 7 on 2026-09-07 (corridor policy; see the row).

**This paragraph said "37 of 57" and was thirteen out, across six of the seven
planes**, which is a larger error than any one of them and is the reason it is
being re-derived rather than nudged. The thirteen, plane by plane, each with
the command in its row above: Ingestion +1 (the deep-web tier, shipped at
`4efd34d`), Cognition +1 (the self-model, which has a production caller in
`platform.rs`), Valuation +4 (four of §16.1's six wired, against a cell that
said none was), Optimisation +1 (family clustering's caller in `stage_learn`,
`76c677f`), Execution +1 (the region table consulted at the cell, `cac99d3`)
and Ledger +5 (a bar correction, not new code — see the row). **Six of the
thirteen were not new work at all**: Valuation's four and Ledger's five were
already in the tree when the old number was written, and two of the six
one-capability moves had a commit this document already cites elsewhere. A
figure re-derived from rows nobody re-read reproduces itself, which is what
happened here for four planes running.

**Where this and `PROJECT-PLAN.md` §1 stand against each other, and why the
agreement is now real rather than arithmetical.** As of 2026-09-07 both tables
read **54 of 57** over the **same seven numerators**: Ingestion 4/5 · Cognition
7/7 · Valuation 5/6 · Intelligence 7/7 · Optimisation 5/5 · Execution 15/15 ·
Ledger 11/12. Every one of those terms was re-derived from code in the same
pass rather than copied from the other document, and each row above carries the
`grep` that proves its own figure.

**The state this replaces is the one worth recording, because it is a failure
mode and not a typo.** Both tables previously totalled 53 of 57 by *different*
memberships: this one had Cognition 6/7 and Ledger 12/12, §1 had Cognition 7/7
and Ledger 11/12. The two differences were one apiece in opposite directions
and cancelled exactly, so a reader who did the arithmetic found it correct in
both documents and concluded they agreed about the platform — while they
disagreed about two of its seven planes. **A cancelling disagreement is worse
than a visible one**, and it survived at least three re-scores, twice being
written up as an irresolvable difference of definition rather than resolved.
It was resolvable both times: Cognition's item counted a `Belief` stage that
does not exist and will not, and Ledger's twelfth counted a policy table under
a word that means an enforced boundary. Neither needed new code; both needed
somebody to read the item's own name against the tree.

Both are now closed in the same pass, in opposite directions and each recorded
separately — Cognition 6 → 7, Ledger 12 → 11 — precisely so that a future
reader can see the two moves rather than a net change of zero. Intelligence's
former difference (§1 at 6/7 against this table's 7/7) was simple staleness and
§1 has since been re-derived to 7/7; it is not a disagreement any more.

**Do not treat the agreement as corroboration.** Two documents that were
re-derived from the same tree in the same session by the same method are one
measurement written twice, not two. The evidence for any figure is the command
in its row, and disputing a row means running that command — not observing that
the other document says the same thing. §1's composite — **57/71 from
2026-09-07**, 56/71 before it, 54/71 before that — stitches its own count of
these 57 to two layers from §2.3 (UI 3/7, Infrastructure 0/7) and moves with
this re-derivation rather than independently of it.

The prior figures this paragraph carried, kept because the trend is the useful
part and the levels were wrong: 36 of 57 at `e04815e`,
32 of 52 at `584c96b`, 31 of 52 at `296e187` and 25 of 50 at `de5d042` —
**each of them measured the same way as the "37" and should be read as
floors.** At
...
capability equally, which flatters nothing and nothing in particular: an
unpriced corporate action and an unbuilt per-user ledger are both "one".
...
|---|---|---|---|---|
| 1 Experience | sign-up surface; identity call; passkeys; customer mandate; product entitlements; per-user account; Leptos | 3 of 7 | 3/7 | **Re-scored 2026-09-07, up from 1 of 7. This cell was stale by two and both closures predate it**; [`rescore-2026-09-06.md`](rescore-2026-09-06.md) §"UI / Experience (Layer 1) — 3 of 7" scored it correctly on 2026-09-06 and `PROJECT-PLAN.md` §1's UI row has read 3/7 since `bcf7330`, so **this row was the older measurement and the outlier of three documents, not the dissenting one.** It also never named which one of the seven it was counting, which is why it could sit wrong for a wave: an unnamed numerator cannot be checked. Scored item by item, each verified here rather than carried. **Counted (3): sign-up surface** — `frontend/portal/src/app/(auth)/sign-up/page.tsx` with `tests/auth.spec.ts` (`grep -c '^\s*test(' frontend/portal/tests/auth.spec.ts` → 8). **These are passes, not declarations, as of 2026-09-07:** `npx playwright test tests/auth.spec.ts tests/treasury-mandates.spec.ts tests/treasury-products.spec.ts` reported `16 passed (33.1s)`, including `the whole journey: sign up, verify, sign in, use the portal, sign out`. The row was scored on counted `test(` declarations when it was re-scored earlier the same day, and said so on its face; the run converts it. A count of declarations is not a count of passes, which this repository has already recorded once about its own hashing tests. **Customer mandate** — the type is `qip_capital::ledger::mandate::Mandate`, served by `GET /ledger/users` (`grep -n 'pattern: "/ledger' backend/crates/apps/qip-api/src/routes.rs`), rendered by `treasury/ledger/page.tsx` and the mandate register `treasury/mandates/page.tsx`, with `tests/treasury-mandates.spec.ts` → 4. **Product entitlements** — `treasury/products/page.tsx` (`bcf7330`) with `tests/treasury-products.spec.ts` → 4. **Not counted (4), each for a stated reason rather than by omission: identity call** — IMPLEMENTED-UNVERIFIED, and `1b87de7` moved it without closing it: three specs in `tests/identity-provider.spec.ts` (`grep -c '^\s*test(' …` → 3) now hold the Identity Platform branch's *refusal* when a project is named and the key is missing, which was the half that could fail silently — it previously fell one edit away from authenticating people against a local JSON password store. That commit's own message states what it does not establish: "that a real Identity Platform sign-in succeeds. That needs a project, a key, and an observed deployment." A tested refusal is not a tested call. **Passkeys** — MISSING; ADR 0038 is *proposed* and its closing section is "Nothing is applied", blocking on four checks only the owner can run. **Per-user account** — MISSING, and the page's existence is the trap: `treasury/accounts/page.tsx` renders an account an *operator* chose by `?user=`, never the signed-in person's own, because the gateway forwards one deployment credential and `Principal` carries a bearer subject and no `UserId`. `grep -n 'pattern: "/ledger\|pattern: "[^"]*account' backend/crates/apps/qip-api/src/routes.rs` returned exactly three lines on 2026-09-07 — `/ledger/users`, `/ledger/users/:user/eligibility`, `/ledger/users/:user/investment-requests` — and no route resolving a session to a ledger account. The surface is built and the capability is not. **Leptos** — MISSING; ADR 0025 is a record with no code. Flow 1: page TESTED, identity IMPLEMENTED-UNVERIFIED; constraint row §2.1 CONTRADICTS (Next.js). Phase 13. **3 of 7 is the TESTED bar — a named passing Playwright spec — and Playwright was not run for this re-score**; the specs are counted by declaration and the runs are the frontend lane's evidence, cited as theirs. Nothing here is deployed either |
...
| 3 Application and API | documented endpoints exist; K3's narrower reach is what is built; per-user API; typed-intent surface (§40.9) | 3 of 4 | 3/4 | `documentation.rs::every_documented_endpoint_exists`; K3's reach is now a test rather than a reading — `api_boundary.rs::the_application_layer_depends_on_no_execution_venue_capital_or_edge_crate`, `::the_api_uses_only_the_centre_half_of_the_mesh_and_none_of_its_service_clients` (`827a40e`). **The typed-intent surface is TESTED, 2026-09-07, up from 2 of 4.** §40.9 gives `investment-api` one intent and forbids it an order; `POST /ledger/users/:user/investment-requests` raises it through `Platform::decide_investment`, which was the first production caller of `UserLedger::admit` — the mandate gate (eligibility, entitlement, currency, investable capital net of what is at work, the share tolerated at one strategy) had been written, tested in `qip-capital` and reachable from nothing a deployed process ran, which is the `MaxExpectedShortfall` shape. Both outcomes are journalled under `kernel/investment` and neither moves a book; the body carries a constant `funded: false` and `api_boundary.rs::every_mutating_route_is_reviewed_here_and_each_raises_a_typed_intent` now asserts `platform.fund_user(` appears in no source of the crate. Evidence: `qip-kernel/tests/ledger.rs::a_request_past_what_the_mandate_leaves_investable_is_refused_by_name_and_both_outcomes_are_journalled`, `::a_request_naming_a_family_the_factory_did_not_register_the_strategy_under_is_refused_rather_than_re_labelled`, `::an_operator_credential_older_than_the_kernel_accepts_raises_no_investment_request`; `qip-api/tests/ledger_routes.rs::an_operator_raises_an_investment_request_and_the_route_answers_the_limit_that_refused_it`, `::a_viewer_cannot_raise_an_investment_request_and_a_user_with_no_mandate_is_not_a_decision`, `::an_investment_request_amount_sent_as_a_number_or_carrying_a_funding_field_is_refused`. **The fourth stays open and this cell's old reason for it was wrong**: it said `qip-api` "composes reads only, none per-user", and two per-user routes were live when that was written (`/ledger/users`, `/ledger/users/:user/eligibility`) with a third now. The true gap is narrower and is stated here instead: authorisation is by *role*, not by user. `Principal` carries a subject from a deployment bearer token and no `UserId` (`qip-api/src/auth.rs`), so no route can serve a caller their own account and none tries; a per-user API in §40.9's sense waits on the identity store layers 1 and 2 list as MISSING, and is not closable in the application layer alone |
| 4 Domain contracts and control fabric | signed payload down; cell verification; atomic swap; §6.2 narrowing wired; outcome return; twelve producers; two independent halt wires; per-region reservation | 7 of 8 | 7/8 | **Re-scored 2026-09-07, up from 6 of 8, and the correction is about this document rather than about new code: per-region reservation was installed in the composition root at `63e4556` and this row had not been re-read since.** `qip-edge-node/src/allocation.rs` reads `QIP_REGION_ALLOCATION`, refuses absent, blank, unparseable and non-positive with `EX_CONFIG`, and `qip_edge_node::assemble` takes the resulting `RegionCapital` **by value**, so a node cannot be assembled without one; `lib.rs:123` calls `.with_unfunded_region(allocation.amount())`. TESTED at the root: `qip-edge-node/tests/allocation.rs::a_node_built_by_this_root_refuses_to_start_without_a_region_allocation`, `::a_node_built_with_the_allocation_refuses_a_second_strategy_once_it_is_spent` — `test result: ok. 2 passed; 0 failed` on 2026-09-07. The greps the rows below quote as empty (`with_region_allocation`, `RegionAllocation` under `backend/crates/apps/`) are still empty and still prove nothing: the constructor the node calls is `Cell::with_unfunded_region`, so the search term was wrong rather than the wiring absent. What remains true is that no node is deployed (`execution_nodes = {}` in every environment), which is layer 6's zero and not this row's. | Flow 3 verdict paragraph and `qip-api/tests/mesh.rs`; **4 of 12 payload slots have producers — corrected 2026-09-07 from 3**, slot 4's having landed at `ece7602` after this cell was last written (PARTIAL; count with `grep -n 'Slot::produced\|= episodic' backend/crates/apps/qip-api/src/mesh.rs`, not from this cell); two halt wires that share no failure since `ff86473` — a flag polled on the node beside the signed broadcast, `qip-edge-node/tests/halt.rs::the_node_halts_the_cell_on_a_present_flag_and_releases_it_when_the_flag_is_removed`, flow 6 re-traced at `584c96b` — TESTED at the cell and the node and reached by no running node; F6 CONTRADICTS |
| 5 Data and state | source→facts; entity resolution; world event; bitemporal, bounded, hash-chained log; a `Ledger` per §43.3; central strategy books settled from cell reports; those books settled from venue fills and not placements; live source sustained; BigQuery derived series; content-hash manifests | 8 of 10 | 8/10 | Flow 2 links TESTED; `truth_loop.rs`; the books — `qip-kernel/tests/attribution.rs` (`7ef6063`), new since `de5d042`; **the `Ledger` per §43.3 is TESTED, corrected 2026-09-06 from "PARTIAL by naming"** — the books are keyed on `(UserId, StrategyId)` (`qip-capital/src/ledger/book.rs:79,160`) with a production caller at `qip-api/src/mesh.rs:1387`; what they do not carry is position quantity, only attributed cash, and that is the limit rather than the absence this cell used to assert; added absent at `e04815e` and TESTED at `5290bb9` — the books are settled from the delta's `fills` and a sent order settles nothing (`central/plane.rs:1161`; `qip-kernel/tests/attribution.rs::a_report_from_a_cell_older_than_the_fill_record_is_counted_sent_and_settles_nothing`, `qip-api/tests/mesh.rs::an_order_a_cell_reports_sent_and_unfilled_reaches_no_book_and_charges_nothing`; B22, struck); **the tenth — content-hash manifests — is TESTED as of 2026-09-07, up from deferred, so this row is 8 of 10.** The discipline existed for exactly one artefact (`CatalogueManifest`, `qip-financial/src/catalogue.rs`, sha256 of the catalogue text) and for no ingested document, while `NewsItem` is an `EventBody` — so every news item the platform ingested was copied verbatim into the hash-chained log, which is permanent and sealed against edit, a vendor's `Restricted` non-displayable prose with it. Blueprint §7.2 and §56.4 rules 34 and 36 ask for the opposite. `NewsItem::body: String` is now `NewsItem::manifest: SourceManifest` (`qip-financial/src/manifest.rs`) — source, locator, byte length, SHA-256 of the document as served, and the instant it was read — so the type system holds the line rather than a reviewer: there is no `String` on the record for an adapter to archive an article into. Produced in production by `NarrativeAdapter::manifest_for` on the fetch path (`qip-market-ingestion/src/narrative.rs`) and by `SourceManifest::generated` in the synthetic stream, and proven by `qip-market-ingestion/tests/narrative_feed.rs::the_article_text_reaches_no_record_the_log_seals_and_a_hashed_reference_stands_in_for_it`, which asserts against the serialised record rather than the absence of a field. `EventBody` schema version 1 → 2, because a v1 payload is a copy of a document and a v2 payload is a reference to one. The two still open are **live source sustained** — blocked on a deployment and an egress proxy, not on code (§2.1) — and **BigQuery derived series**, which is built-and-unwired rather than absent: `qip_storage::gcp::BigQueryWarehouse` handles both ways BigQuery reports failure without failing, and `grep -rn BigQueryWarehouse backend/crates --include=*.rs` finds no constructor outside `qip-storage`'s own tests, so wiring it needs a composition root and a project |
...
Summed: **32 of 53 layer items at the TESTED bar** (3 + 3 + 3 + 7 + 8 + 0 +
8 over 7 + 4 + 4 + 8 + 10 + 7 + 13) — re-added from the rows on 2026-09-07,
**three times on that date**, the third time moving layer 1 from 1 to 3 when
this row was checked against `rescore-2026-09-06.md` and `PROJECT-PLAN.md` §1
and found to be the only one of the three still reading 1. Layers 3, 4, 5 and
7 were each re-verified from code in that pass and are unmoved: layer 3's
typed-intent surface (`grep -n 'investment-requests' backend/crates/apps/qip-api/src/routes.rs`,
and `decide_investment` in `platform.rs`), layer 4's per-region reservation
(`grep -n 'with_unfunded_region\|RegionCapital' backend/crates/apps/qip-edge-node/src/lib.rs`)
and its payload slots (`grep -n 'Slot::produced\|= episodic' backend/crates/apps/qip-api/src/mesh.rs`
returned four produced slots of twelve on 2026-09-07), and layer 5's
content-hash manifests (`grep -n 'manifest: SourceManifest' backend/crates/libs/qip-financial/src/intelligence.rs`,
one hit, and no `body: String` beside it). **32 of 53 is a repository bar and
not an operating one**: layer 6 is zero because nothing has been applied and
observed, and layer 7's five absent items are the collection gap — no scrape
has been observed anywhere. Earlier on the same date: the first re-addition found the layer 5 term reading `6`
against a row that read 7 of 10 and the row then moving to 8 of 10 when
content-hash manifests landed; the second moved layers 3 and 4, the first
because §40.9's typed-intent surface was built and the second because
per-region reservation had been installed at `63e4556` and the row had not
been re-read. **Three lanes therefore edited this line in one day and two of
the three moves were rows nobody had re-read rather than code that landed,
which is the argument for adding it from the rows every time rather than
adjusting the previous number** — up from 25 of 53 at `e04815e`, where
...
| B12 | **Per-region reservation table** (F6) — **CLOSED, verified 2026-09-07. Everything after this sentence in this cell is the superseded record and is kept, not deleted, because the way it was wrong is the useful part: it quotes a grep for `RegionAllocation` under `backend/crates/apps/` and reports it empty, and the grep is still empty because the constructor the node calls is `Cell::with_unfunded_region`. A search term that finds nothing is not an absence.** The ledger landed at `0ca4b92` and the composition root at `63e4556`: `qip-edge-node/src/allocation.rs` reads `QIP_REGION_ALLOCATION` and refuses absent, blank, unparseable and non-positive with `EX_CONFIG`; `qip_edge_node::assemble` takes the resulting `RegionCapital` by value, so no node can be built without one. `qip-edge-node/tests/allocation.rs::a_node_built_by_this_root_refuses_to_start_without_a_region_allocation` and `::a_node_built_with_the_allocation_refuses_a_second_strategy_once_it_is_spent` — `test result: ok. 2 passed; 0 failed` on 2026-09-07. Superseded record follows: still absent. **Re-verified after wave 5 (`da0789d`) and the infrastructure work through `2e19a4c`: unchanged.** `grep -n -i reserv backend/crates/edge/qip-edge/src/cell.rs` finds nothing but "preserved"; `grep -rn -i reserv backend/crates/apps/qip-edge-node/src` and `grep -rln -i "per.region\|per_region" backend/` (excluding the unrelated ADR-0008 comment and capital-fabric doc comment) find no per-region reservation mechanism anywhere in `backend/crates/edge` or `backend/crates/runtime/qip-kernel`. Wave 5's deployment work (strategies installed from the payload) did not touch this; it is a different mechanism | §4.2, §26, §33, rule 21 | P3 | — | None in code | A disconnected cell refusing its own second proposal against one envelope; the central ledger unchanged; `apps/qip-edge-node/tests/mesh.rs` extended | 2 |
...
| 7 Episodic and belief | Calibration within tolerance; sizing responds | `qip-agents/src/memory.rs`; `bayes.rs`; the calibration is now a Brier score on `qip_belief_brier_score` (`04738ee`); **corrected 2026-09-07:** this cell said "no belief stage in the cycle, so sizing does not respond" and both halves were wrong. There is no `Belief` stage and there will not be one (`cycle.rs` has eight arms, settled); belief is formed in REASON, graded in LEARN, and *does* reach sizing through `BeliefFreshness::assess` in `central_degradation`, whose multiplier narrows the free budget. §2.2's Cognition row has said so since 2026-09-06. Slot 4 is also not empty: `grep -n 'episodic_digest =' backend/crates/apps/qip-api/src/mesh.rs` finds the producer | PLANNED; payload slot 3 empty, slot 4 produced | Phase 3 gate |
...
| 9 Self-model and exploration | Value of information measured | **Corrected 2026-09-07: this cell read "Nothing (`grep -rln SelfModel` empty)" and its own command disproves it** — `grep -rln "SelfModel" backend/crates` returns eight files, among them `qip-learning-engine/src/self_model.rs`, `qip-api/src/self_model_views.rs` and `qip-kernel/src/platform.rs`. §2.2's Cognition row has said so since 2026-09-06 ("one is composed and called in production"), so this table contradicted the same document two hundred lines above it. What is genuinely absent is the *value-of-information* measure this row's deliverable names, not the self-model | Deliverable MISSING; the self-model it presumed absent is built and composed | Phase 8 gate |
...
| 14 Valuation plane | Fixed income and options live | **Corrected 2026-09-07, was "Nothing":** the kernel composes the plane at `qip-kernel/src/valuation.rs`, and `CreditRegister::from_universe` is called from `platform.rs` (`grep -n 'CreditRegister::from_universe' backend/crates/runtime/qip-kernel/src/platform.rs`). §1 scores the plane 5 of 6. Options are the part that is genuinely absent, by decision: ADR 0050 refuses the volatility surface a caller until an option-quote source clears a licensing evaluation | Fixed income and credit BUILT and composed; options MISSING by ADR 0050 | Phase 2 gate |
...
| 17 Illiquid and private | Positions marked with method | **Corrected 2026-09-07, was "Nothing":** `qip_financial::valuation::IlliquidValuator::mark_object` is called from `platform.rs` inside `private_holdings_of`, the marks are held on the platform and read at sizing, and `ValuationMethod` — this row's own deliverable, by name — is exported from `qip-financial/src/lib.rs`. Locate with `grep -n 'IlliquidValuator\|mark_object' backend/crates/runtime/qip-kernel/src/platform.rs` | BUILT and reached; not exercised by any deployment, like everything else here | Phase 14 |
...
> **Correction, 2026-09-07 — the paragraph above is false and the grep in it
> was the reason.** `63e4556` installed the allocation in
> `qip-edge-node`'s composition root, and the search still returns nothing
> because the root calls `Cell::with_unfunded_region` and holds a
> `RegionCapital`, neither of which contains either search term. A node
> cannot be assembled without one — `assemble` takes it by value — and
> `qip-edge-node/tests/allocation.rs` drives both halves. The paragraph is
> kept rather than rewritten because it is the second entry in this document
> to reach a false conclusion from an empty grep, and the shape of the
> mistake is worth more than the row.
