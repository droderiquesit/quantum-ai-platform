...
|---|---|---|---|---|
| 1 Ingestion | absorb records; entity resolution; licensing before use; one live source sustained; deep-web tier | 4 of 5 | 4/5 | `absorption.rs`, `sense.rs`, `qip-fastbrain/src/licensing.rs`; live source is PARTIAL (one tick, no deployment) and is the only thing keeping this row below 5. **Corrected 2026-09-06, up from 3 of 5:** this cell said "deep-web tier MISSING" and the tier model has existed since `4efd34d`. `ls backend/crates/services/qip-data-finder/src/tier.rs` finds it; `grep -c '#\[test\]' backend/crates/services/qip-data-finder/tests/tiers.rs` printed `16`; `grep -n tier backend/crates/services/qip-data-finder/src/finder.rs` finds the `use crate::tier::{…}` and the assessment that asks the tier "before any request is made". `PROJECT-PLAN.md` §1's Ingestion row has read 4/5 since that commit and this row did not follow it |
| 2 Cognition | world model; causal graph; episodic memory; hypotheses; belief stage in the cycle; counterfactuals with a production caller; self-model | 6 of 7 | 6/7 | **Corrected 2026-09-06, up from 5 of 7: this cell said "no self-model" and one is composed and called in production.** `grep -n 'SelfModel\|self_model' backend/crates/runtime/qip-kernel/src/platform.rs` finds the import from `qip_learning_engine::self_model`, the field on `Platform`, its construction, `self.self_model.absorb(evaluation, &components)` and `self.self_model.origin_factors()` feeding `set_origin_factors` — a caller, not a registration. `understanding.rs`, `reasoning.rs`, `world.rs:41`, `causal.rs:234`; belief still SUBSTITUTED (flow 2) though now graded — `learning.rs::a_cycle_that_resolves_a_thesis_grades_it_and_moves_the_calibration_series` (`04738ee`); counterfactuals gained their caller — `learning.rs::a_refused_order_is_priced_once_its_horizon_has_passed_and_charged_to_its_gate` (`b9e2242`), up from 4 of 7. **The one that keeps this row at 6 of 7 is "belief stage in the cycle", and it is where this table and `PROJECT-PLAN.md` §1 (which reads 7/7) genuinely disagree rather than one being stale.** They are not counting the same item. `sed -n '/pub enum Stage/,/^}/p' backend/crates/runtime/qip-kernel/src/cycle.rs` returns eight arms — `Sense`, `Understand`, `Discover`, `Reason`, `Simulate`, `Decide`, `Act`, `Learn` — and no `Belief` among them; belief is graded *inside* LEARN, by `calibrate_resolved` called from `stage_learn` (`grep -n 'calibrate_resolved' backend/crates/runtime/qip-kernel/src/platform.rs`). PROJECT-PLAN's row names its capability "belief" and counts the grading; this row names it "belief stage in the cycle" and counts the stage, which is what SUBSTITUTED has always meant here. Neither number is wrong; the difference is one row's wording and is stated rather than split |
| 3 Valuation | six engines (§16.1) | 4 of 6 | 4/6 | **Corrected 2026-09-06, up from 0 of 6, and this is the largest single error §2.2 carried.** The cell said "no engine prices anything"; four of §16.1's six are wired into the kernel and reachable by command. `grep -rn 'CreditRegister::from_universe\|CreditProfile::from_object\|IlliquidValuator::mark_object\|CommitmentBook\|VolatilitySurface' backend/crates/runtime/qip-kernel/src/` finds *term structure* (`CreditRegister::from_universe` called from `platform.rs`), *credit* (`CreditProfile::from_object` called from `valuation.rs`), *illiquid valuation* (`IlliquidValuator::mark_object` called from `platform.rs`) and *cashflow and commitments* (`CommitmentBook` held as a field on `Platform` and constructed there) — and finds the *volatility surface* only inside a doc comment, which is not a caller. *Corporate actions* remain absorbed as a world event and not priced. So 4 of §16.1's six, and the two absent ones are named. [`rescore-2026-09-06.md`](rescore-2026-09-06.md) (committed at `74ef5bd`) established this and also established that **this row's denominator was the wrong six** — it had substituted the liquidity ladder, which §16.1 does not name, for corporate actions, which it does; the ladder is genuinely wired and is a seventh thing, counted nowhere here. `PROJECT-PLAN.md` §1 already tabulates this row's old 0/6 as one of four defensible readings (the 70.42% line); with this correction the two documents read the same 4/6 and the "MISSING-CURRENT, as `completion-plan.md` still scores the plane" row in that table is now describing a state this file has left |
...
**Where this and `PROJECT-PLAN.md` §1 still differ, and why.** This table now
totals **53 of 57** and §1 totals 52: both moved to Optimisation 5/5 on
2026-09-07 when the multi-horizon gate gained a cycle-stage caller at
`27da0c5`, and this table then moved Intelligence to 7/7 on the same day,
because `27da0c5` also gave corridor policy its subject and only this row
was re-read. §1's numerators are Ingestion 4/5 · Cognition 7/7 ·
Valuation 4/6 · Intelligence 6/7 · Optimisation 5/5 · Execution 15/15 ·
Ledger 11/12; this table's are the same except **Cognition 6/7, Intelligence
7/7 and Ledger 12/12**. **The totals now differ by one and the memberships
differ in three places, so the visible gap understates the real one and the
sum is still the thing to distrust.** Until 2026-09-07 the totals *agreed*
while the memberships did not, because Cognition and Ledger differed by one in
opposite directions and cancelled exactly: a reader checking the arithmetic
found it correct in both documents and concluded they agreed about the
platform, which they did not. Intelligence is the third difference and the
only one that is simply staleness — §1's 6/7 is the older measurement of the
same item, and it should be re-derived rather than argued with.
...
`sed -n '/pub enum Stage/,/^}/p' backend/crates/runtime/qip-kernel/src/cycle.rs`
returns eight arms with no `Belief` among them. Quote whichever you mean, and
say which. **The second is the Ledger plane's twelfth**, and it is the reason
the totals coincide rather than a reason to trust them: this table's row 7 now
reads 12/12 because its twelfth — the §43.3 per-user per-strategy ledger — was
verified present from code on 2026-09-06 (`LedgerKey = (UserId, StrategyId)`,
`qip-capital/src/ledger/book.rs:79`; production caller `qip-api/src/mesh.rs:1387`),
while §1's row still reads 11/12 against a twelfth this table does not count at
all — custody as an *enforced boundary*, which ADR 0021 refuses with no expiry
and which therefore cannot move without an owner decision. So one document has
a capability it scores as absent and cannot build, the other has one it scored
as absent and had already built. Adding the two 51s together would double-count
nothing and mean nothing. §1's composite — 56/71 since 2026-09-07, and 54/71 before it — stitches its own count of these 57 to two layers from
§2.3 (UI 3/7, Infrastructure 0/7) and is unaffected by this re-derivation
except through that one disputed row.
...
Summed: **30 of 53 layer items at the TESTED bar** (1 + 3 + 3 + 7 + 8 + 0 +
8 over 7 + 4 + 4 + 8 + 10 + 7 + 13) — re-added from the rows on 2026-09-07,
twice on that date. The first re-addition found the layer 5 term reading `6`
against a row that read 7 of 10 and the row then moving to 8 of 10 when
content-hash manifests landed; this one moves layers 3 and 4, the first
...
been re-read. Two lanes therefore edited this line in one day, which is the
argument for adding it from the rows every time rather than adjusting the
previous number — up from 25 of 53 at `e04815e`, where
