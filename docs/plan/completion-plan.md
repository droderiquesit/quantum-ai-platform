...
ending at `5290bb9`, with one test-only helper in `e2e_live.rs` still being
corrected. **Unblocked today:** B8, B10's central half, B12; B13's owner
half and B23 wait on the owner.
...
before the field existed replays as having confirmed nothing. The one thing
left of it is a test-only helper in `e2e_live.rs` that still reads positions
from `orders`, being corrected as this is written. Of the seven alignment
items, five are spent; A6 still waits on an attested sidecar digest nobody
has pinned; and A5's desk half has closed on its own terms — the whitelist is
produced and shipped, the node runs passes — leaving only the normaliser's
disposition to the owner. The boundaries are still enforced structurally, the
application layer's reach is still a test rather than a reading, the
paper-trading line still has three layers and a test on each, and the venue
feed is a fourth refusal in the same register — `QIP_VENUE_FEED` accepts one
value and names ADR 0003 for any other. The cloud layer is unchanged: the
runtime in the tree is the blueprint's, never seen by a Terraform binary,
never planned, never applied, and no process has ever been shown to run on
it. Call it one slice from aligned — the owner's sentence on the normaliser —
with one layer that cannot be scored above zero from this environment, and
every edge and wire control TESTED and measured nowhere, because no node is
deployed.
