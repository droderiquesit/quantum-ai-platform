...
| Cloud | GCP, as Terraform: Cloud Run for the three central binaries and the two frontends, a Compute Engine execution node per region (`execution_nodes = {}` in every environment's tfvars), Secret Manager as mounted files, KMS, Binary Authorization, WIF. `dev` has been applied by `infra.yml`'s `up`, dispatched by a person — `infrastructure/CLAUDE.md` records the observation. The GKE cluster, edge-cell and console-ingress modules, the Helm chart, the raw manifests and the Argo CD stack were removed at `808ca32`, `67b3e92` and `7d79161`. An earlier version of this row said no `terraform` binary existed here; one does (`/usr/local/bin/terraform`, `Terraform v1.9.8`, read on 2026-09-04), so `terraform fmt -check` and `terraform validate` run locally. A plan still needs the environment's state bucket and a credential this checkout does not hold, so a plan is `infra.yml`'s to produce |
| Pipelines | `ci.yml`, `deploy.yml`, `infra.yml`, `vendor.yml` — all deriving identity from committed tfvars (`ls .github/workflows` = 4 at `bb47119`) |
| Decision records | 27 ADRs, `0001`–`0027`, every one listed in `docs/adr/README.md` (`ls docs/adr \| grep -c '^[0-9]'` = 27 at `e04815e`). Three of them, `0025`, `0026` and `0027`, are proposed rather than accepted: each frames a decision for the owner and marks its recommendation as one. `0027` (`360cfd8`) asks what a concentration cap is a share of, filed because the default set's share-of-gross caps refuse the first order into the instrument catalogue every central root now loads (`8224509`) |
