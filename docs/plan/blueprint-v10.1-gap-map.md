...
> 0 of 6 engines to 6 of 6 built, **4 of 6 with a production caller**; the
...
| Liquidity ladder (rung-by-rung withdrawal ordering, cash → PE commitments) | **BUILT-UNWIRED** | Re-scored 2026-09-05 at `1107305`; previously ABSENT. `qip-financial/src/ladder.rs:238` defines `LiquidityLadder`. Monotonicity is compared by cross-multiplication so money never leaves `Decimal`, and the rung order is the enum's declaration order — a property of the type rather than of a sort call someone can forget. **No production caller**, and the reason is structural rather than pending: the blueprint's caller is serving a withdrawal from the top of the ladder, which needs a *granted* withdrawal, and ADR 0021 leaves `WithdrawalEntitlement` with exactly one arm, `Refused` — no `Granted` to construct and no `Deserialize` to smuggle one through (`qip-capital/src/ledger/entitlement.rs:155` constructs `WithdrawalEntitlement::refused()` unconditionally). Its one admissible seam is a risk read — how much of the book becomes cash within a horizon — and that was deliberately not wired in `1107305`. `grep -rl LiquidityLadder backend/crates --include=*.rs` returns only `qip-financial/src/{lib,ladder}.rs` and `qip-financial/tests/ladder.rs`. The ladder is execution-adjacent by subject and inert by construction: its plan legs carry an object id, a rung, an amount and a cost, and a test asserts the serialised field set is exactly those four so a future venue or side field trips it |
...
23 + 4 + 8 + 9 = 44, and the one excluded row is the "eight execution paths"
...
- **35 of 44** identified capabilities have *any* production code resembling
  them (BUILT+WIRED, BUILT-UNWIRED, or PARTIAL) — 23 + 4 + 8. It would be 36
...
- **23 of 44** identified capabilities have a wired production caller
  (BUILT+WIRED only — code that exists and is reached from a composition root
  or from a service the kernel actually calls). Of those 23, several are
...
- **4 of 44** are BUILT-UNWIRED, and the two that joined that category at
  `1107305` are the more interesting half of the valuation result: the
  volatility surface and the liquidity ladder are built, tested, and reached
  by nothing, each for a reason that is not "nobody got to it yet" — no
  option-quote source has passed a licensing evaluation, and a granted
  withdrawal is a thing ADR 0021 makes unconstructible. Neither is scored as
  wired, and a future re-score should check the caller rather than the type.
...
- **Liquidity ladder** — the type exists; the *ordering it would serve* does
  not, because serving a withdrawal needs a granted withdrawal and ADR 0021
  leaves `WithdrawalEntitlement` with one arm. Built, unwired, and unwirable
  along its blueprint seam.
