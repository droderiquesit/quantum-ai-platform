...
| **Ledger, wallet, treasury** | 92% at the record-and-refuse bar | 11/12 TESTED | per-user per-strategy ledger with mandates, entitlements and a withdrawal that cannot be built, booked from the centre's settlements for a desk user (`0599092`, 2026-09-05; 12 mutations); destination registry, corridor lifecycle, seven-veto transfer gate, wallet read model with halting reconciliation, and custody policy as data (`5546a24`; 75 mutations) — all as ADR 0021 permits: records and refusals, nothing that moves, signs or calls out; fills attributed to contributors with zero residual (`7ef6063`); centre bills from venue fills, not placements, after the B22 wire-billing defect closed (`5290bb9`); a retired strategy's open positions are now dispositioned and the review runs from the LEARN stage in a deployed cycle (`c77877f`, 2026-09-04) | A mandate registry under the desk's ceiling, an `InvestmentRequest` admitted or refused by a named limit, pro-rata fill splits that reconcile exactly, and a fabric journal whose every wallet, corridor and gate decision replays from the hash-chained log exist as of 2026-09-05, but the kernel still books every fill to the desk whole and constructs no `FabricJournal`; no transfer engine exists and none may (ADR 0021), so the gate's Approved has no consumer; the twelfth capability, custody as an enforced boundary rather than a policy record, is Phase 12 |
...
first, 141 commits. Date and time and the first line are read from
