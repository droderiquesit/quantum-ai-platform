...
|---|---:|---|---|---|
| **Ingestion** (SENSE, licensing, dedup) | 60% | 3/5 named capabilities TESTED | `qip-market-ingestion` absorption (`absorption.rs`), one licensing catalogue for every root (`qip-data-finder/src/admission.rs`; the fast brain's private table deleted, `48665b1`), Frankfurter registered in the connector bridge (`9f4a557`) and moved with the vendor to `api.frankfurter.dev` in manifest, bootstrap and allowlist together (`48665b1`, 2026-09-04, a live fetch through the connector released three observations); `IngestionService` made constructible by a composition root (`26fa4f4`); the demonstration tape is schema 2 with bitemporal macro releases, alt-data readings and a dividend declaration (`48665b1`) | Two live sources (Coinbase keyless, Frankfurter) exist and were each exercised in-session through a TLS-terminating bridge, never for a sustained period through a deployed egress proxy (Phase 1 exit unmet); the deep-web source tier does not exist |
...
first, 131 commits. Date and time and the first line are read from
