...
| 18.4 | Compounding Policy | PARTIAL | **Row repaired 2026-09-19: it carried six cells in a four-column table** — the pre-ADR-0066 evidence, a second `PARTIAL` verdict, and then two near-identical copies of the re-scored evidence. A reader met the superseded claim first, and that claim was false by then: it said `grep -rn 'reinvest\|Reinvest\|withdrawal_drag\|minimum_viable_scale' backend/crates --include=*.rs` "returns nothing", and the command printed 38 lines on 2026-09-19. **Scored `PARTIAL` (ADR 0066).** `CompoundingPolicy`, `ReinvestmentDecision`, `ThresholdLadder` and a bounded `FeeVolumeLedger` are built, all money `Decimal`, reached through the cadence fold. Two gaps hold it short. First, the fee-tier ladder is unreached, and **more of it is unreached than this row used to say**: not only does `grep -rn 'FeeTier' backend/crates --include=*.rs` find the definition in `edge/qip-routing/src/venue.rs`, one re-export and test callers only, but `grep -rn 'VenueProfile::listed' backend/crates --include=*.rs` and `grep -rn 'FeeSchedule::tiered' backend/crates --include=*.rs` are **both test-only**, so the whole venue profile — not just its ladder — has no production constructor, and the only schedule anything builds is the flat one `VenueProfile::listed` supplies. `qip_capital::compounding::FeeVolumeLedger` deliberately holds the trailing volume and not the rungs, so the join needs a caller holding both, and no crate outside `qip-routing` composes a `qip_routing::FeeSchedule` at all. Second, withdrawal drag and minimum viable scale are **not built**, because both need a forward growth rate the platform does not measure and inventing one would be a second claim about a fact nothing establishes. |
