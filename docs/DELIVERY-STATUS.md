...
| 43.2 | Valuation | PARTIAL | All ten objects now exist; three are read by nothing. **This row said "six of the ten" and undercounted by four, because it grepped the blueprint's names rather than the tree's** — search by definition, not by identifier. `YieldCurve` is `TermStructure`, whose module opens "Term structures: yield curves, forward curves" and which carries the discount factors and forwards the blueprint asks for: `grep -n 'pub struct TermStructure\|pub fn discount_factor\|pub fn forward_rate' backend/crates/libs/qip-market/src/curve.rs`. `VolSurface` is `VolatilitySurface`: `grep -n 'pub struct VolatilitySurface' backend/crates/libs/qip-market/src/volatility.rs`. `LogisticsCost` is `LandedCost` over `LogisticsTerms`, carrying freight, duty, clearance, storage, marketplace fees and spoilage — the blueprint's definition item for item: `grep -n 'pub struct LandedCost\|pub struct LogisticsTerms\|pub struct Spoilage\|pub struct MarketplaceFees' backend/crates/libs/qip-financial/src/physical.rs`. `CapitalCall` was the one genuinely absent object and now exists, with the consequence of failure the blueprint names as part of it: `grep -n 'pub struct CapitalCall\|pub enum CallConsequence' backend/crates/libs/qip-financial/src/cashflow.rs`. **Six of the ten are read by non-test code — seven until 2026-09-19, when the seventh was re-examined and found to be the `MaxExpectedShortfall` shape rather than a read; see the `CapitalCall` paragraph at the end of this cell.** `TermStructure` and `CreditProfile` from `grep -n 'use qip_market::curve::\|use qip_financial::credit::' backend/crates/runtime/qip-kernel/src/valuation.rs`; `AssetValuation` (which carries `ValuationMethod` and `ValuationInput`) and `CorporateAction` from `grep -n 'illiquid_marks\|corporate_actions_pending' backend/crates/runtime/qip-kernel/src/platform.rs`; `Commitment` from `grep -n 'CommitmentBook::new\|Commitment::from_private_asset' backend/crates/runtime/qip-kernel/src/platform.rs`; `AuctionState` from `grep -n 'AuctionState' backend/crates/edge/qip-orderbook/src/venue.rs`; and the unfunded balance itself through `Commitment::obligation`, which `CommitmentBook::unfunded_total` sums and which the kernel subtracts from free capital before anything is sized (`grep -n 'unfunded_total' backend/crates/runtime/qip-kernel/src/platform.rs` returned four lines, three of them calls and the fourth a comment inside the `#[cfg(test)]` module — read them, do not count them). **The three that are read by nothing are why this stays PARTIAL, and none is an oversight.** `VolatilitySurface` is deliberately without a caller: ADR 0050 settles that nothing may feed it until an option-quote source passes the licensing gate, and explicitly rejects fitting it to the platform's own numbers (`grep -n 'Synthetic\|alternatives' docs/adr/0050-what-an-option-quote-source-must-satisfy-before-the-volatility-surface-has-a-caller.md`). Do not wire it to close this row. `CashflowForecast` is constructed only in tests: `Commitment::from_private_asset` goes through `unscheduled`, which sets the forecast to `None`, and `grep -rn '\.with_forecast(' backend/crates --include=*.rs \| grep -v /tests/` returns only an unrelated `DemandForecast` builder in the pre-positioner and a doc comment, so no production commitment carries a pacing model. `LandedCost` has no reader at all — `grep -rn 'LandedCost\|PhysicalArbitrage' backend/crates --include=*.rs \| grep -v qip-financial` returns nothing — and a physical-arbitrage valuation caller belongs in a service or the kernel rather than in the lib that defines it. **`CapitalCall` is the tenth object and it is not read in production, which is a stronger statement than the one this row made until 2026-09-19 and the reason the count above dropped from seven to six.** The old wording — "its read path reaches production, its write path does not yet" — is the optimistic half of a true sentence. `Commitment::obligation` is `unfunded() + accrued_default_penalty()`, and `accrued_default_penalty` iterates `self.calls`, a map nothing outside tests ever inserts into: `grep -rn 'record_call\|CapitalCall::notice' backend/crates --include=*.rs \| grep -v /tests/` returned four lines on 2026-09-19, every one of them inside `qip-financial/src/cashflow.rs` — the two `record_call` definitions, one doc line, and the book's delegation to the commitment — and not one of them a caller from outside the module that defines the type. So the penalty term is structurally zero on every cycle the platform has ever run, and no `CapitalCall` is ever constructed for the read path to read. That is the `MaxExpectedShortfall` pattern exactly — a figure summed into a capital reservation, reading as a control, unable to be anything but zero — and the register should name it as such rather than record the read as reached. **The remaining work is not where this row said it was, and it is not small.** The old cell put the seam "alongside `Commitment::from_private_asset`". Nothing there can build a notice without inventing one: `PrivateAssetDetails` carries `committed_capital` and `called_capital` — capital already drawn and paid — and no notice reference, no outstanding amount and no due date (`grep -n 'pub struct PrivateAssetDetails' -A 14 backend/crates/libs/qip-financial/src/extensions.rs`), while `CapitalCall::notice` requires the fund's own reference and its doc says why it is required rather than generated: "two calls for the same amount on the same date are ordinary, and a book that cannot tell them apart will silently hold only one". A generated reference would be a second source of truth for a notice nobody issued. Nor can it arrive by ingestion today: `grep -n 'pub enum SensedRecord' -A 14 backend/crates/services/qip-market-ingestion/src/adapter.rs` has no variant carrying one, and `ReferenceDataUpdate` is a single `field`/`new_value` string pair, so routing a structured notice through it would mean parsing one out of a string. So the genuine options are two, and both are decisions rather than patches: an operator intake on `Platform` in the shape of `record_capital_demand`, whose caller would have to be a route in `qip-api`; or a new ingested record type with a source behind it. The first is the smaller and is where `capital_call_notice_days` — a serialised `PrivateAssetDetails` field whose own doc records that nothing reads it — would finally earn a reader. |
