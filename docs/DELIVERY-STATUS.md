...
| 8.2 | Why the Graph Structure Earns Its Place | PARTIAL | The traversal primitives every listed query needs are built and tested: `grep -n 'fn paths_between\|fn reachable\|fn neighbours\|fn predecessors\|fn degree' backend/crates/services/qip-world-model/src/graph.rs`, plus causal shock propagation `grep -n 'fn propagate' backend/crates/services/qip-world-model/src/causal.rs`, tested at `ls backend/crates/services/qip-world-model/tests/understanding.rs`. One of the five named queries has a production caller — the shortest causal path from a driver to an instrument, through the panel: `grep -n 'causal.propagate' backend/crates/agents/qip-investment-agents/src/reasoning.rs` reached by `stage_reason → chief → CausalAnalyst` (`grep -n 'CausalAnalyst::new' backend/crates/agents/qip-investment-agents/src/chief.rs`). **Corrected 2026-09-15, and the correction runs the understating way: this cell called four queries unbuilt and cited that search as returning nothing.** Run it — `grep -rn 'hidden_concentration\|second_order\|exposed_to' --include=*.rs backend/crates` answers at length, across five files, and three of the four have code. **Hidden concentration across held positions is not merely built, it is production-reached by a chain this document already credits elsewhere.** `grep -n 'pub fn hidden_concentration' backend/crates/services/qip-world-model/src/exposure.rs` is entered through the kernel's pass-through `grep -n 'pub fn concentration' backend/crates/runtime/qip-kernel/src/causal_review.rs`, which `causal_review::review` assembles and `grep -n 'fn causal_review' backend/crates/runtime/qip-kernel/src/platform.rs` calls from `stage_understand` on every cycle — the same seam §9.1 credits for confounders. Two-hop instrument exposure and second-order dependency on unheld entities are built, tested and called by nothing outside a test: `grep -rn 'instruments_exposed_to\|unheld_dependencies' --include=*.rs backend/crates` finds the two functions, their re-export, `qip-world-model/tests/exposure.rs` and the module's own `#[cfg(test)]`, and no other caller. Only the fourth is genuinely not built — nothing resolves a prediction contract on an entity's event, and `grep -rni 'prediction_contract' --include=*.rs backend/crates` returns nothing. So two of the five named queries have a production caller, two are `UNREACHED`, and one has no code. The row stays `PARTIAL`, on three queries rather than four, and a lane acting on the old cell would have rebuilt `hidden_concentration` beside a copy that already runs every cycle. |
...
| 27.2 | Rules Across Venues and Cycles | PARTIAL | Four of five rows built and reached: grouping is by instrument, venue **and** representation, so a different venue and a different representation each get their own order, and a cycle leg is isolated by type — `grep -n 'struct NettingKey\|NettingPolicy::NoNet\|pub fn net(' backend/crates/libs/qip-contracts/src/intent.rs`; the no-net flag cannot be forgotten because `CycleLeg` has no nettable form (`grep -n 'From<CycleLeg>' backend/crates/libs/qip-contracts/src/intent.rs`). Missing: the router consolidating unspecified-venue intents onto the best venue, and the market-making row (quotes skewing off the net) — `grep -rn 'consolidat' --include=*.rs backend/crates/edge/qip-routing/` returns nothing — that half of the claim is correct as written. **The other half is corrected 2026-09-15, understating**: "there is no quoting engine (see §29.1)" cites a row that has read `REACHED` since 2026-09-14. There is one. `grep -n 'quote_loop::review' backend/crates/runtime/qip-kernel/src/platform.rs` prints two lines, both exits of `stage_act`, and it computes a fair value, a decomposed half spread and an inventory skew per instrument. What keeps this row's market-making seat empty is a narrower thing than the absence of an engine: the loop skews off the position the platform actually holds, not off the netting seam's output, and `QuotePair` deliberately carries no venue and no side, so nothing skews a quote off the net or could place one if it did. |
...
| 36.3 | Failure Isolation | PARTIAL | Four of six rows built and production-reached: an unreachable venue is excluded from action (`grep -n 'VenueStatus::Unreachable' backend/crates/edge/qip-edge/src/cell.rs`); a halted cell stops without stopping its peers, under three independent halt sources (`grep -n "fn halt(\|names::EDGE_HALTED" backend/crates/edge/qip-edge/src/telemetry.rs`); central-unavailable degrades to the last shipment behind a circuit breaker rather than spooling (`grep -n 'CircuitBreaker\|CircuitOpen' backend/crates/edge/qip-edge/src/mesh.rs`); and a degraded model pauses only the strategy classes that depend on it, cutting size rather than stopping everything (`grep -n 'fn pauses\|fn sizing_multiplier\|enum Capability' backend/crates/libs/qip-contracts/src/degradation.rs`). **Corrected 2026-09-15, understating.** This cell called both remaining rows absent and cited that search as returning nothing; `grep -rni 'mirror' --include=*.rs backend/crates/edge/` answers over a dozen files, two of which are named `mirror.rs`. **The stale-reference row is built and it refuses.** A reference read past its window is a hard denial inside `grep -n 'pub fn direction_gate' backend/crates/edge/qip-routing/src/mirror.rs`, which cannot return a direction without clearing `DistributedReference::indication`'s window first; `grep -n 'CHECK_REFERENCE_FRESH\|CHECK_DIRECTION_PERMITTED' backend/crates/edge/qip-routing/src/extension.rs` is the path-3 arm that will not report satisfied unless both halves cleared, and the cell calls it per mirrored leg at `grep -n 'fn check_extension_for' backend/crates/edge/qip-edge/src/cell.rs`. It is `UNREACHED` and not absent, for §31.1's reason and not for want of code: `grep -rn 'install_mirror(' --include=*.rs backend/crates` finds the method and test callers only, so no cell on a production path holds a mirror that could have a stale reference. What is genuinely not built is the **dark-region** arm — `grep -n 'pub enum RegionState' -A 18 backend/crates/edge/qip-routing/src/mirror.rs` names four states, every one of them a position against an inventory band and none of them "this region has gone dark", and nothing suspends an arrangement on one. Four rows reached, one built and unreached, one absent. |
...
| 39.1 | Where AI, Quantum and Language Models Sit | PARTIAL | Held rows — "risk and transfer enforcement: deterministic Rust, no model" is structural (`grep -n 'Determinism::Required' backend/crates/services/qip-cost-router/src/router.rs`), and "quantum plus classical" carries a non-optional baseline (`grep -n 'pub classical_baseline\|needs_a_classical_baseline' backend/crates/libs/qip-quantum/src/benchmark.rs`). The "cost, dispersion, regime estimation — ONNX models, shipped, in-process" row is **decided in ADR 0083 (2026-09-19)**: in-process, yes, by an in-tree provider for the four forms the platform trains; ONNX, retired as a name. This row said the row "cannot be built under the two-dependency rule" until then, which confused the format with the mechanism. `grep -rni 'onnx' backend/crates --include=*.rs \| grep -v '/tests/'` still returns one prose mention in `qip-contracts/src/policy.rs` and no implementation, and the advisory consumer — a served score feeding a deterministic filter, never a threshold or a size — has no caller yet, so the row is decided and not reached. Language-model rows exist only as the agent panel's budgeted capability (`grep -n 'pub enum Capability' backend/crates/libs/qip-agents/src/capability.rs`), not as a filings/news hypothesis reader. |
...
| 45.1 | Google Cloud | PARTIAL | Most of the catalogue is declared; a handful of rows have nothing, and nothing is deployed. Declared: `grep -rhno 'resource "google_[a-z_]*"' infrastructure/terraform --include=*.tf | sort -u` finds Spanner, BigQuery, Storage, `google_redis_instance` (Memorystore), Pub/Sub, one VPC, Cloud Router/NAT, `google_service_networking_connection` (PSC), Secret Manager, KMS, WIF pool and provider, Artifact Registry, Binary Authorization, SCC modules, and the C3 execution node. Absent entirely: `grep -rn 'google_compute_security_policy\|google_workflows_workflow\|google_cloud_hsm\|guest_accelerator' infrastructure/terraform --include=*.tf` returns nothing — no Cloud Armor, no Cloud Workflows, no Cloud HSM, no spot GPU; no Firebase Cloud Messaging resource either. Cloud Run is no longer Terraform's: `grep -n 'google_cloud_run_v2_service' infrastructure/terraform/modules/cloudrun/main.tf` finds only the comment saying ADR 0036 moved it to Config Connector manifests under `infrastructure/gitops/envs/`. Cloud Build and Cloud Deploy are GitHub Actions instead (`ls .github/workflows`). **Amended 2026-09-14 (ADR 0069).** The Cloud Armor / load balancer / CDN row is now declared. **Cloud Workflows, Cloud HSM and spot GPU are deliberately not, and that is a decision rather than an omission:** each is a capability whose consumer does not exist — no lifecycle orchestration calls Workflows, no asymmetric key material of any kind exists anywhere (ADR 0043 records that gap), and `enable_vertex_ai = false` in every environment means no training job would use a GPU. Declaring infrastructure nothing consumes produces a plan showing a resource created, which is not evidence of anything. ADR 0069 records the reversal condition for each as a grep for a caller. |
