...
| 22.2 | Sufficient Statistics | PARTIAL | **Re-scored 2026-09-19 from two of the seven rows to three and a half. The clause reading "Absent: t-digest/KLL quantiles, HyperLogLog, weighted reservoir sampling" is now false in all three limbs, and the "Built-not-reached: Welford" clause with it**; the grep it rested on returns four declarations — `grep -rn 'pub struct \(TDigest\|HyperLogLog\|Reservoir\|RunningStats\)' backend/crates/libs/qip-numerics/src`. All four reached production by one seam: `grep -n 'feature_statistics::measure' backend/crates/runtime/qip-kernel/src/platform.rs` returns two lines, one a comment and one the single call, inside `Platform::stage_understand` over `self.price_history`, so the estimators summarise every priced instrument on every cycle, and `grep -n 'moments\|digest\|distinct\|sample' backend/crates/services/qip-training/src/estimators.rs` shows `FeatureSummary` composed of exactly Welford, the t-digest, the HyperLogLog and the weighted reservoir. Proven to have measured rather than merely been called by `a_cycle_over_a_long_enough_series_summarises_it_with_all_four_streaming_estimators` in `backend/crates/runtime/qip-kernel/tests/platform_services.rs`, which requires strictly ordered 5th/50th/95th percentiles from the digest, more than one distinct value from the HyperLogLog, a positive standard deviation from Welford and a non-empty reservoir — figures a constructed-but-unfed estimator cannot produce. **Row by row.** *Quantiles* — reached, above. *Frequency and cardinality* — reached twice over: the HyperLogLog above, and the count-min sketch in the deep brain's research campaign with the error bound declared as a value the sketch is built from (`grep -n 'pub struct ErrorBound\|pub struct CountMinSketch\|pub const MAX_COUNTERS' backend/crates/libs/qip-numerics/src/sketch.rs`, `grep -n 'CountMinSketch::new\|tolerable_for' backend/crates/apps/qip-deepbrain/src/campaign.rs`), which refuses the fit when the declared error at the counted volume exceeds five percent of the desk's minimum — §56.3's rule 30, with a consumer. *Representative samples* — reached; `FeatureEstimators::standard` weights the reservoir by arrival index, which is the recency weighting as arithmetic rather than as a setting, though at 64 rows rather than a population stand-in. *Mean, variance, higher moments* — **half**, and the half that is missing is the one the section names: `RunningStats` is online for count, mean, variance, stddev, min and max (`grep -n 'pub fn ' backend/crates/libs/qip-numerics/src/stats.rs` inside `impl RunningStats`), but it is not "extended for skew and kurtosis" — `grep -n 'pub fn skewness' backend/crates/libs/qip-numerics/src/stats.rs` is a batch free function over a slice, which is the opposite of a streaming moment. *Covariance and correlation* — not met. `grep -n 'stats::covariance' backend/crates/runtime/qip-kernel/src/platform.rs` computes it in production but batch, from a retained return series, and the exponentially weighted online update the row asks for has no production caller at all: `grep -rn 'ewma(' backend/crates --include=*.rs \| grep -v /tests/ \| grep -v qip-numerics/src/stats.rs` returns nothing. *Linear and logistic models* — absent; `grep -rn 'recursive_least_squares\|RecursiveLeastSquares\|OnlineGradient' backend/crates --include=*.rs` returns nothing. *Factor structure* — absent; `grep -rn 'fn oja\|StreamingPca\|IncrementalSvd' backend/crates --include=*.rs` returns nothing. The memory claim the table makes is measured rather than asserted: `the_reported_memory_is_the_summary_s_and_does_not_move_with_the_series` compares a 400-price series against a 40,000-price one and requires equal summary bytes inside 8 KB, so "few KB per distribution" is a number this repository checks. |
