...
the test confirmed to fail for the stated reason, the code restored
byte-for-byte with `cmp`, the test reconfirmed passing — twenty-two
mutations in all, one of which (`from_document` skipping `validate`) was
run with `--no-fail-fast` so all three roots' tests were seen firing.
