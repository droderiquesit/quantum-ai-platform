...
commit that introduced or changed the test it fires — one of them run as
an unprivileged user, because uid 0 ignores the mode bits and the
read-only property is otherwise unprovable on a root build host. `git
