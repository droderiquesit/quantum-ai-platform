...
| 43.1 | World and Cognition | PARTIAL | **13 of the 14 objects exist under some name — re-counted 2026-09-19, and this cell was understating by one in exactly the way the tree keeps catching.** `EntityRelation` was listed as missing on the strength of `grep -rn 'pub struct EntityRelation' backend/crates --include=*.rs` returning nothing, but the blueprint defines it as "a directed relationship: owns, supplies, competes, depends on, is collateral for, settles" and the tree has exactly that under its own name: `grep -n 'pub struct Relationship\|pub enum RelationshipKind' backend/crates/services/qip-world-model/src/relationship.rs` — `from`/`to` give the direction, `RelationshipKind` enumerates `SubsidiaryOf`, `Supplies`, `Competitor`, `DerivesFrom`, `CounterpartyOf` and `ResolvedBy` among others, and `weight` and `source` carry the strength and the provenance. A name grep for the blueprint's spelling is not a search for the object. The other twelve: `grep -rn 'pub struct CausalEdge\|pub struct CausalGraph' backend/crates/services/qip-world-model/src/causal.rs`, `grep -rn 'pub struct Episode' backend/crates/libs/qip-agents/src/memory.rs`, `grep -rn 'pub struct CapabilityEstimate' backend/crates/services/qip-learning-engine/src/self_model.rs`, `grep -rn 'pub struct SourceCandidate' backend/crates/services/qip-data-finder/src/source.rs`, `grep -rn 'pub struct DeepWebAdapter' backend/crates/services/qip-data-finder/src/tier.rs`. **One is genuinely absent: `WorldEvent`** — "an observed occurrence linked to entities, with source, event time, receipt time and confidence". `grep -rnE 'pub (struct|enum) (WorldEvent|ObservedEvent)\b' backend/crates --include=*.rs` returns nothing, and nothing in the tree pairs an entity link with both instants. It belongs beside `Relationship` in `qip-world-model`. §43's own framing — "all defined once in the types crate" — is still not met: the thirteen live in eight different crates. |
