...
| 23.7 | Scenario and Stress | PARTIAL | Built and tested: `grep -n 'pub struct StressTester\|pub fn standard_library' backend/crates/services/qip-simulation-engine/src/scenario.rs` covers historical replay and correlation stress with a scenario library and per-position impact. **A production caller now exists.** `qip-kernel`'s `Platform::stress_test_book`, called from `stage_simulate` every cycle with an open position, applies the standard library's first scenario to the book as it stands, at a conservative full (beta 1.0) equity-factor exposure absent a fitted per-name beta — crediting a diversification nobody measured would understate the risk this control exists to surface — and publishes the loss fraction on `qip_simulation_stress_loss_fraction`: `grep -rn 'StressTester\|standard_library()' backend/crates --include=*.rs` now finds the call in `qip-kernel/src/platform.rs` beside the crate's own tests. Two of the four methods are still not built at all: causal propagation through mechanisms and adversarial worst-plausible-sequence construction, which is why this is `PARTIAL` and not `REACHED`. |
...
overclaiming: `qip-deepbrain`'s `NetworkProbe`-backed discovery pass refuses
every call by name until a TLS-capable transport is authorised (ADR 0009), so
it is real and in the binary rather than actually assessing anything today;
`qip-kernel`'s `capacity_probe` and `stress_test_book` both declare their
zero-factor and full-beta scope in their own doc comments and in the two new
metric descriptions beside `qip_portfolio_effective_bets`'s own; the
...
- **Reviewed and narrowed (round six, `e691804`, the current head)** —
...
- **A reporting defect in round five's own evidence, corrected** — the
  ...
  satisfy it. The corrected report quotes two runs that were made —
  deleting the parameter-region cut gives `9 of 28 redaction rows are
  wrong`, reversing the ordering gives `4 of 28`, including
  `http://127.0.0.1:9105/x?a=1@2&api_key=SECRETVALUE` coming back as
  `http://…@2&api_key=SECRETVALUE`.
...
`e691804` (round six, the current head), and this entry's commit. None
of the three has been pushed. Each of the three code commits touches
