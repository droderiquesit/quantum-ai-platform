...
- **An execution node running on GCE C3 and holding venue sessions.** At that
  point the blueprint's central topological claim is real, GKE is carrying the
  part it was never meant to carry, and step 5 becomes overdue rather than
  premature.
...
Until that decision is recorded here, the sequence above is the only permitted
direction of travel, and no step in it is taken without the evidence in its row.
