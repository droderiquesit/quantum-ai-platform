...
| [0029](0029-the-normaliser-is-removed-rather-than-recorded-as-research-only.md) | `qip-normalization` is deleted rather than recorded research-only: nothing constructed it, its unmapped-symbol guard could not fire, and four documents cited it as a control — the gap it leaves is named here rather than disguised; *the decision is taken, the removal is not yet applied* |
