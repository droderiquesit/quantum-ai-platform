...
dispatched — GitHub knows of four workflows and `image` is not one of them —
and `image_bake_subnet_cidr` is commented out in `environments/dev/`, so the
module creates nothing. A bake exists; no image does.
